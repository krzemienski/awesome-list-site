...
Run: `2026-09-17T23-41-41-280Z-28370`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2008740529` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-17T23:41:00.000Z` on both sides.  
Denominator: 1 pass / 1 fail / **2** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only artifact.showcase --width 375,768`): 2 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| artifact.showcase | 375 | pixel | FAIL | 1125080 | 15.5058% | 375×19333 vs 375×19349 | [actual](/actual/artifact.showcase-375.png) · [expected](/expected/artifact.showcase-375.png) · [diff](/diff/artifact.showcase-375.png) — 15.506% differing pixels exceeds the 0.5% ceiling |
