...
Run: `2026-09-18T05-10-49-263Z-10039`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2040312753` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-18T05:10:00.000Z` on both sides.  
...
Selected run (`--only app.about,app.resource.detail --width 375,1440`): 4 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.about | 375 | pixel | FAIL | 86133 | 2.8950% | 375×7934 vs 375×7798 | [actual](/actual/app.about-375.png) · [expected](/expected/app.about-375.png) · [diff](/diff/app.about-375.png) — 2.895% differing pixels exceeds the 0.5% ceiling |
| app.about | 1440 | pixel | FAIL | 379503 | 6.2585% | 1440×4211 vs 1440×4017 | [actual](/actual/app.about-1440.png) · [expected](/expected/app.about-1440.png) · [diff](/diff/app.about-1440.png) — 6.258% differing pixels exceeds the 0.5% ceiling |
| app.resource.detail | 375 | pixel | FAIL | 1020896 | 55.6271% | 375×4894 vs 375×2394 | [actual](/actual/app.resource.detail-375.png) · [expected](/expected/app.resource.detail-375.png) · [diff](/diff/app.resource.detail-375.png) — 55.627% differing pixels exceeds the 0.5% ceiling |
| app.resource.detail | 1440 | pixel | FAIL | 3266055 | 65.8564% | 1440×3444 vs 1440×1345 | [actual](/actual/app.resource.detail-1440.png) · [expected](/expected/app.resource.detail-1440.png) · [diff](/diff/app.resource.detail-1440.png) — 65.856% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `sub="across 9 categories"` | `sub="across 10 categories"` | nav category count |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","3d ago"],["categories","9","96 KB","30d ago"],["subcategories","92","56 KB","30d ago"],["users","8","80 KB","0s ago"],["resource_audit_log","6,283","2.7 MB","26m ago"],["enrichment_jobs","26","168 KB","3d ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 3ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 121d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
