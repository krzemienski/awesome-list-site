...
Run: `2026-09-16T23-24-08-370Z-4588`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2026366195` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 4 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T23:24:00.000Z` on both sides.  
Denominator: 2 pass / 2 fail / **4** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.admin.enrichment,app.admin.researcher --width 375,768`): 4 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.enrichment | 375 | pixel | PASS | 562 | 0.1058% | 375×1416 vs 375×1416 | [actual](/actual/app.admin.enrichment-375.png) · [expected](/expected/app.admin.enrichment-375.png) · [diff](/diff/app.admin.enrichment-375.png) |
| app.admin.enrichment | 768 | pixel | PASS | 490 | 0.0623% | 768×1024 vs 768×1024 | [actual](/actual/app.admin.enrichment-768.png) · [expected](/expected/app.admin.enrichment-768.png) · [diff](/diff/app.admin.enrichment-768.png) |
| app.admin.researcher | 375 | pixel | FAIL | 3705 | 0.8159% | 375×1211 vs 375×1211 | [actual](/actual/app.admin.researcher-375.png) · [expected](/expected/app.admin.researcher-375.png) · [diff](/diff/app.admin.researcher-375.png) — 0.816% differing pixels exceeds the 0.5% ceiling |
| app.admin.researcher | 768 | pixel | FAIL | 5926 | 0.7535% | 768×1024 vs 768×1024 | [actual](/actual/app.admin.researcher-768.png) · [expected](/expected/app.admin.researcher-768.png) · [diff](/diff/app.admin.researcher-768.png) — 0.754% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `sub="2 admins · 1 contributor"` | `sub="7 admins · 5 contributors"` | /api/admin/users roles (admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","41h ago"],["categories","9","96 KB","29d ago"],["subcategories","92","56 KB","29d ago"],["users","12","80 KB","0s ago"],["resource_audit_log","6,223","2.7 MB","6h ago"],["enrichment_jobs","26","168 KB","41h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 7ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 120d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
