...
Run: `2026-09-15T05-45-43-063Z-15042`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2025117793` "Nick" (torn down: local row deleted, Clerk user deleted, 1 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-15T05:45:00.000Z` on both sides.  
...
Selected run (`--only app.home.index,app.category,app.resource.detail --width 375`): 3 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
| app.resource.detail | 375 | pixel | FAIL | 728711 | 45.8742% | 375×4236 vs 375×2386 | [actual](/actual/app.resource.detail-375.png) · [expected](/expected/app.resource.detail-375.png) · [diff](/diff/app.resource.detail-375.png) — 45.874% differing pixels exceeds the 0.5% ceiling |
...
Catalog snapshot `247d136b3019657d…` (9 categories, 1816 resources) bound to the design's `AV_*` globals; admin globals bound: AV_RETAINED_OPERATIONS, AV_RETAINED_CATALOG, AV_TOTAL_USERS, AV_USERS, AV_RECENT_ACTIVITY, AV_PENDING_APPROVALS, AV_PENDING_RESOURCES, AV_AUDIT_LOGS, AV_AUDIT_TOTAL, AV_CONTACT_SUBMISSIONS.
