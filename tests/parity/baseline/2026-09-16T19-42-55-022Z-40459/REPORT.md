...
Run: `2026-09-16T19-42-55-022Z-40459`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2025194533` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 1 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T19:42:00.000Z` on both sides.  
...
| app.admin.users | 1024 | pixel | FAIL | 8263 | 0.7349% | 1024×1098 vs 1024×1098 | [actual](/actual/app.admin.users-1024.png) · [expected](/expected/app.admin.users-1024.png) · [diff](/diff/app.admin.users-1024.png) — 0.735% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 7ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 119d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
