...
Run: `2026-09-16T23-04-18-231Z-73944`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2090088709` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 2 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T23:04:00.000Z` on both sides.  
Denominator: 0 pass / 1 fail / **1** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only artifact.showcase --width 375`): 1 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| artifact.showcase | 375 | pixel | FAIL | 3007279 | 19.8533% | 741×20442 vs 741×20442 | [actual](/actual/artifact.showcase-375.png) · [expected](/expected/artifact.showcase-375.png) · [diff](/diff/artifact.showcase-375.png) — 19.853% differing pixels exceeds the 0.5% ceiling |
