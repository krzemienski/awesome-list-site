...
Run: `2026-09-17T23-33-26-527Z-26921`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2081449090` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-17T23:33:00.000Z` on both sides.  
Denominator: 3 pass / 3 fail / **6** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.category,artifact.docs.forms,artifact.showcase --width 375,768`): 6 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.category | 375 | pixel | FAIL | 254114 | 5.9677% | 375×11355 vs 375×11334 | [actual](/actual/app.category-375.png) · [expected](/expected/app.category-375.png) · [diff](/diff/app.category-375.png) — 5.968% differing pixels exceeds the 0.5% ceiling |
| app.category | 768 | pixel | FAIL | 481301 | 6.9788% | 768×8980 vs 768×8716 | [actual](/actual/app.category-768.png) · [expected](/expected/app.category-768.png) · [diff](/diff/app.category-768.png) — 6.979% differing pixels exceeds the 0.5% ceiling |
| artifact.docs.forms | 375 | pixel | PASS | 694 | 0.0610% | 375×3034 vs 375×3034 | [actual](/actual/artifact.docs.forms-375.png) · [expected](/expected/artifact.docs.forms-375.png) · [diff](/diff/artifact.docs.forms-375.png) |
| artifact.docs.forms | 768 | pixel | PASS | 694 | 0.0332% | 768×2725 vs 768×2725 | [actual](/actual/artifact.docs.forms-768.png) · [expected](/expected/artifact.docs.forms-768.png) · [diff](/diff/artifact.docs.forms-768.png) |
| artifact.showcase | 375 | pixel | FAIL | 1206748 | 16.6279% | 375×19353 vs 375×19349 | [actual](/actual/artifact.showcase-375.png) · [expected](/expected/artifact.showcase-375.png) · [diff](/diff/artifact.showcase-375.png) — 16.628% differing pixels exceeds the 0.5% ceiling |
| artifact.showcase | 768 | pixel | PASS | 2441 | 0.0248% | 768×12835 vs 768×12835 | [actual](/actual/artifact.showcase-768.png) · [expected](/expected/artifact.showcase-768.png) · [diff](/diff/artifact.showcase-768.png) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","3d ago"],["categories","9","96 KB","30d ago"],["subcategories","92","56 KB","30d ago"],["users","8","80 KB","0s ago"],["resource_audit_log","6,277","2.7 MB","2h ago"],["enrichment_jobs","26","168 KB","3d ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 2ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 121d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
...
| admin.jsx | `        <Stat label="Last enriched" value="3h ago" sub="batch #21 · 47 entries" />
        ...
        <Stat label="Avg cost" value="$0.34" sub="per batch" />` | `        <Stat label="Last enriched" value="74d ago" sub="batch #38 · 3 entries" />
        <Stat label="Queue" value="0" sub="idle" />
