...
Run: `2026-09-16T05-09-50-845Z-66494`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2128340484` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 8 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T05:09:00.000Z` on both sides.  
...
Selected run (`--only app.admin.audit --width 375,1440`): 2 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.audit | 375 | pixel | FAIL | 320338 | 19.4365% | 375×3623 vs 375×4395 | [actual](/actual/app.admin.audit-375.png) · [expected](/expected/app.admin.audit-375.png) · [diff](/diff/app.admin.audit-375.png) — 19.437% differing pixels exceeds the 0.5% ceiling |
| app.admin.audit | 1440 | pixel | FAIL | 81807 | 1.7331% | 1440×3278 vs 1440×3278 | [actual](/actual/app.admin.audit-1440.png) · [expected](/expected/app.admin.audit-1440.png) · [diff](/diff/app.admin.audit-1440.png) — 1.733% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 16ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 119d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
