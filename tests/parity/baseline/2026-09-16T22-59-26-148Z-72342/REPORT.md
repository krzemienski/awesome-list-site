...
Run: `2026-09-16T22-59-26-148Z-72342`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2092676211` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 2 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T22:59:00.000Z` on both sides.  
Denominator: 0 pass / 6 fail / **6** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.admin.categories,app.admin.subcategories --width 375,768,1024`): 6 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.categories | 375 | pixel | FAIL | 3097 | 0.6792% | 375×1216 vs 375×1216 | [actual](/actual/app.admin.categories-375.png) · [expected](/expected/app.admin.categories-375.png) · [diff](/diff/app.admin.categories-375.png) — 0.679% differing pixels exceeds the 0.5% ceiling |
| app.admin.categories | 768 | pixel | FAIL | 5003 | 0.5998% | 768×1086 vs 768×1086 | [actual](/actual/app.admin.categories-768.png) · [expected](/expected/app.admin.categories-768.png) · [diff](/diff/app.admin.categories-768.png) — 0.600% differing pixels exceeds the 0.5% ceiling |
| app.admin.categories | 1024 | pixel | FAIL | 5253 | 0.5084% | 1024×1009 vs 1024×1009 | [actual](/actual/app.admin.categories-1024.png) · [expected](/expected/app.admin.categories-1024.png) · [diff](/diff/app.admin.categories-1024.png) — 0.508% differing pixels exceeds the 0.5% ceiling |
| app.admin.subcategories | 375 | pixel | FAIL | 7196 | 0.7321% | 375×2621 vs 375×2621 | [actual](/actual/app.admin.subcategories-375.png) · [expected](/expected/app.admin.subcategories-375.png) · [diff](/diff/app.admin.subcategories-375.png) — 0.732% differing pixels exceeds the 0.5% ceiling |
| app.admin.subcategories | 768 | pixel | FAIL | 10550 | 0.6207% | 768×2213 vs 768×2213 | [actual](/actual/app.admin.subcategories-768.png) · [expected](/expected/app.admin.subcategories-768.png) · [diff](/diff/app.admin.subcategories-768.png) — 0.621% differing pixels exceeds the 0.5% ceiling |
| app.admin.subcategories | 1024 | pixel | FAIL | 12832 | 0.6680% | 1024×1876 vs 1024×1876 | [actual](/actual/app.admin.subcategories-1024.png) · [expected](/expected/app.admin.subcategories-1024.png) · [diff](/diff/app.admin.subcategories-1024.png) — 0.668% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 2ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 120d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
