...
Run: `2026-09-16T19-22-55-144Z-36701`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2037749461` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 1 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T19:22:00.000Z` on both sides.  
Denominator: 4 pass / 4 fail / **8** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.admin.users,app.admin.github --width 375,768,1024,1440`): 8 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
| app.admin.github | 1440 | pixel | FAIL | 6680 | 0.5154% | 1440×900 vs 1440×900 | [actual](/actual/app.admin.github-1440.png) · [expected](/expected/app.admin.github-1440.png) · [diff](/diff/app.admin.github-1440.png) — 0.515% differing pixels exceeds the 0.5% ceiling |
...
| app.admin.users | 1024 | pixel | FAIL | 6939 | 0.6172% | 1024×1098 vs 1024×1098 | [actual](/actual/app.admin.users-1024.png) · [expected](/expected/app.admin.users-1024.png) · [diff](/diff/app.admin.users-1024.png) — 0.617% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","37h ago"],["categories","9","96 KB","29d ago"],["subcategories","92","56 KB","29d ago"],["users","9","80 KB","0s ago"],["resource_audit_log","6,223","2.7 MB","2h ago"],["enrichment_jobs","26","168 KB","37h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
