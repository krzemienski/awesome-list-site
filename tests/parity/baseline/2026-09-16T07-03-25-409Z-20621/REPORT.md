...
Run: `2026-09-16T07-03-25-409Z-20621`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2063993363` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 8 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T07:03:00.000Z` on both sides.  
...
Selected run (`--only app.admin.categories,app.admin.subcategories,app.admin.users --width 375,768`): 6 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.categories | 375 | pixel | FAIL | 4298 | 0.9425% | 375×1216 vs 375×1216 | [actual](/actual/app.admin.categories-375.png) · [expected](/expected/app.admin.categories-375.png) · [diff](/diff/app.admin.categories-375.png) — 0.943% differing pixels exceeds the 0.5% ceiling |
...
| app.admin.subcategories | 375 | pixel | FAIL | 8306 | 0.8451% | 375×2621 vs 375×2621 | [actual](/actual/app.admin.subcategories-375.png) · [expected](/expected/app.admin.subcategories-375.png) · [diff](/diff/app.admin.subcategories-375.png) — 0.845% differing pixels exceeds the 0.5% ceiling |
...
| app.admin.users | 375 | pixel | FAIL | 30788 | 4.4212% | 375×1807 vs 375×1857 | [actual](/actual/app.admin.users-375.png) · [expected](/expected/app.admin.users-375.png) · [diff](/diff/app.admin.users-375.png) — 4.421% differing pixels exceeds the 0.5% ceiling |
| app.admin.users | 768 | pixel | FAIL | 95806 | 6.7834% | 768×1748 vs 768×1839 | [actual](/actual/app.admin.users-768.png) · [expected](/expected/app.admin.users-768.png) · [diff](/diff/app.admin.users-768.png) — 6.783% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","25h ago"],["categories","9","96 KB","28d ago"],["subcategories","92","56 KB","28d ago"],["users","16","80 KB","0s ago"],["resource_audit_log","6,221","2.7 MB","6h ago"],["enrichment_jobs","26","168 KB","25h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
