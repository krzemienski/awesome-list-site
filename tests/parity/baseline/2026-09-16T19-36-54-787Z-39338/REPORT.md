...
Run: `2026-09-16T19-36-54-787Z-39338`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2069303231` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 1 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T19:36:00.000Z` on both sides.  
Denominator: 8 pass / 4 fail / **12** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.admin.users,app.admin.github,app.admin.linkhealth --width 375,768,1024,1440`): 12 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.github | 375 | pixel | FAIL | 3240 | 0.8113% | 375×1065 vs 375×1065 | [actual](/actual/app.admin.github-375.png) · [expected](/expected/app.admin.github-375.png) · [diff](/diff/app.admin.github-375.png) — 0.811% differing pixels exceeds the 0.5% ceiling |
...
| app.admin.github | 1024 | pixel | PASS | 2525 | 0.2812% | 1024×877 vs 1024×877 | [actual](/actual/app.admin.github-1024.png) · [expected](/expected/app.admin.github-1024.png) · [diff](/diff/app.admin.github-1024.png) |
...
| app.admin.linkhealth | 375 | pixel | FAIL | 23439 | 5.4732% | 375×1142 vs 375×1087 | [actual](/actual/app.admin.linkhealth-375.png) · [expected](/expected/app.admin.linkhealth-375.png) · [diff](/diff/app.admin.linkhealth-375.png) — 5.473% differing pixels exceeds the 0.5% ceiling |
| app.admin.linkhealth | 768 | pixel | PASS | 1344 | 0.1709% | 768×1024 vs 768×1024 | [actual](/actual/app.admin.linkhealth-768.png) · [expected](/expected/app.admin.linkhealth-768.png) · [diff](/diff/app.admin.linkhealth-768.png) |
...
| app.admin.linkhealth | 1440 | pixel | PASS | 1670 | 0.1289% | 1440×900 vs 1440×900 | [actual](/actual/app.admin.linkhealth-1440.png) · [expected](/expected/app.admin.linkhealth-1440.png) · [diff](/diff/app.admin.linkhealth-1440.png) |
| app.admin.users | 375 | pixel | FAIL | 17213 | 3.5555% | 375×1291 vs 375×1251 | [actual](/actual/app.admin.users-375.png) · [expected](/expected/app.admin.users-375.png) · [diff](/diff/app.admin.users-375.png) — 3.555% differing pixels exceeds the 0.5% ceiling |
| app.admin.users | 768 | pixel | PASS | 2216 | 0.2340% | 768×1233 vs 768×1233 | [actual](/actual/app.admin.users-768.png) · [expected](/expected/app.admin.users-768.png) · [diff](/diff/app.admin.users-768.png) |
| app.admin.users | 1024 | pixel | FAIL | 8143 | 0.7242% | 1024×1098 vs 1024×1098 | [actual](/actual/app.admin.users-1024.png) · [expected](/expected/app.admin.users-1024.png) · [diff](/diff/app.admin.users-1024.png) — 0.724% differing pixels exceeds the 0.5% ceiling |
| app.admin.users | 1440 | pixel | PASS | 4442 | 0.3110% | 1440×992 vs 1440×992 | [actual](/actual/app.admin.users-1440.png) · [expected](/expected/app.admin.users-1440.png) · [diff](/diff/app.admin.users-1440.png) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","38h ago"],["categories","9","96 KB","29d ago"],["subcategories","92","56 KB","29d ago"],["users","9","80 KB","0s ago"],["resource_audit_log","6,223","2.7 MB","2h ago"],["enrichment_jobs","26","168 KB","38h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 3ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 119d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
