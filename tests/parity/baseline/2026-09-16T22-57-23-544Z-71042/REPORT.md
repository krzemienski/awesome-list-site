...
Run: `2026-09-16T22-57-23-544Z-71042`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 0)  
Identity: disposable Clerk admin `2086535417` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 2 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T22:57:00.000Z` on both sides.  
Denominator: 4 pass / 0 fail / **4** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only artifact.docs.buttons,artifact.docs.forms --width 375,1440`): 4 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
