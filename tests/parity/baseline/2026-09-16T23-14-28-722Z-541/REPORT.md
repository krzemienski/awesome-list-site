...
Run: `2026-09-16T23-14-28-722Z-541`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2069039257` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 3 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T23:14:00.000Z` on both sides.  
...
Selected run (`--only artifact.showcase,artifact.docs.buttons --width 375,1440`): 4 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
| artifact.showcase | 375 | pixel | FAIL | 3624489 | 23.6491% | 741×20683 vs 741×20442 | [actual](/actual/artifact.showcase-375.png) · [expected](/expected/artifact.showcase-375.png) · [diff](/diff/artifact.showcase-375.png) — 23.649% differing pixels exceeds the 0.5% ceiling |
| artifact.showcase | 1440 | pixel | FAIL | 885744 | 5.3459% | 1440×11506 vs 1440×11378 | [actual](/actual/artifact.showcase-1440.png) · [expected](/expected/artifact.showcase-1440.png) · [diff](/diff/artifact.showcase-1440.png) — 5.346% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `sub="2 admins · 1 contributor"` | `sub="6 admins · 5 contributors"` | /api/admin/users roles (admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","41h ago"],["categories","9","96 KB","29d ago"],["subcategories","92","56 KB","29d ago"],["users","11","80 KB","0s ago"],["resource_audit_log","6,223","2.7 MB","6h ago"],["enrichment_jobs","26","168 KB","41h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 5ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 120d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
