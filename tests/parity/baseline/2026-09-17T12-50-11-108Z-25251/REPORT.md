...
Run: `2026-09-17T12-50-11-108Z-25251`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 0)  
Identity: disposable Clerk admin `2088296561` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 1 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-17T12:50:00.000Z` on both sides.  
Denominator: 3 pass / 0 fail / **3** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.subcategory --width 375,1024,1440`): 3 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.subcategory | 375 | pixel | PASS | 4812 | 0.1186% | 375×10823 vs 375×10824 | [actual](/actual/app.subcategory-375.png) · [expected](/expected/app.subcategory-375.png) · [diff](/diff/app.subcategory-375.png) |
| app.subcategory | 1024 | pixel | PASS | 13100 | 0.2390% | 1024×5352 vs 1024×5352 | [actual](/actual/app.subcategory-1024.png) · [expected](/expected/app.subcategory-1024.png) · [diff](/diff/app.subcategory-1024.png) |
| app.subcategory | 1440 | pixel | PASS | 16159 | 0.2847% | 1440×3941 vs 1440×3941 | [actual](/actual/app.subcategory-1440.png) · [expected](/expected/app.subcategory-1440.png) · [diff](/diff/app.subcategory-1440.png) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","2d ago"],["categories","9","96 KB","29d ago"],["subcategories","92","56 KB","29d ago"],["users","9","80 KB","0s ago"],["resource_audit_log","6,260","2.7 MB","1h ago"],["enrichment_jobs","26","168 KB","2d ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
