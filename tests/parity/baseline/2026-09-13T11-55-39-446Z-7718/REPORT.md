...
Run: `2026-09-13T11-55-39-446Z-7718`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2027091288` "Nick" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-13T11:55:00.000Z` on both sides.  
...
Selected run (`--only app.home.curated --width 375`): 1 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.home.curated | 375 | pixel | FAIL | — | — | — | capture failed: page.addStyleTag: Event |
...
Catalog snapshot `253c4a00fa3de86e…` (9 categories, 1816 resources) bound to the design's `AV_*` globals; admin globals bound: AV_TOTAL_USERS, AV_USERS, AV_RECENT_ACTIVITY.
