...
Run: `2026-09-13T20-11-05-577Z-10961`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 0)  
Identity: disposable Clerk admin `2069260054` "Nick" (torn down: local row deleted, Clerk user deleted, 2 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-13T20:11:00.000Z` on both sides.  
...
Selected run (`--only app.home.index --width 1024,1440`): 2 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
Catalog snapshot `8792e8b8c160998d…` (9 categories, 1817 resources) bound to the design's `AV_*` globals; admin globals bound: AV_TOTAL_USERS, AV_USERS, AV_RECENT_ACTIVITY.
