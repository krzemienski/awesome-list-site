...
Run: `2026-09-18T01-26-48-109Z-46597`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2087056981` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-18T01:26:00.000Z` on both sides.  
...
Selected run (`--only artifact.showcase,app.category --width 375,768`): 4 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","3d ago"],["categories","9","96 KB","30d ago"],["subcategories","92","56 KB","30d ago"],["users","8","80 KB","0s ago"],["resource_audit_log","6,277","2.7 MB","4h ago"],["enrichment_jobs","26","168 KB","3d ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 4ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 121d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
