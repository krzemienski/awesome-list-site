...
Run: `2026-09-13T20-39-52-532Z-13171`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2042393692` "Nick" (torn down: local row deleted, Clerk user deleted, 2 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-13T20:39:00.000Z` on both sides.  
Denominator: 11 pass / 1 fail / **12** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.home.curated,app.shell.palette,app.shell.mobile-drawer --width 375,768,1024,1440`): 12 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
| app.shell.palette | 375 | pixel | FAIL | 12910 | 0.5598% | 375×6149 vs 375×6150 | [actual](/actual/app.shell.palette-375.png) · [expected](/expected/app.shell.palette-375.png) · [diff](/diff/app.shell.palette-375.png) — 0.560% differing pixels exceeds the 0.5% ceiling |
| app.shell.palette | 768 | pixel | PASS | 13614 | 0.4130% | 768×4292 vs 768×4292 | [actual](/actual/app.shell.palette-768.png) · [expected](/expected/app.shell.palette-768.png) · [diff](/diff/app.shell.palette-768.png) |
| app.shell.palette | 1024 | pixel | PASS | 17598 | 0.4582% | 1024×3751 vs 1024×3751 | [actual](/actual/app.shell.palette-1024.png) · [expected](/expected/app.shell.palette-1024.png) · [diff](/diff/app.shell.palette-1024.png) |
| app.shell.palette | 1440 | pixel | PASS | 17335 | 0.3774% | 1440×3190 vs 1440×3190 | [actual](/actual/app.shell.palette-1440.png) · [expected](/expected/app.shell.palette-1440.png) · [diff](/diff/app.shell.palette-1440.png) |
...
Catalog snapshot `80b1c260362ed2e9…` (9 categories, 1817 resources) bound to the design's `AV_*` globals; admin globals bound: AV_TOTAL_USERS, AV_USERS, AV_RECENT_ACTIVITY.
