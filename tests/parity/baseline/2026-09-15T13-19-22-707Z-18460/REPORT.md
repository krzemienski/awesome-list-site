...
Run: `2026-09-15T13-19-22-707Z-18460`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 0)  
Identity: disposable Clerk admin `2066995525` "Nick" (torn down: local row deleted, Clerk user deleted, 3 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-15T13:19:00.000Z` on both sides.  
...
Selected run (`--only app.home.index,app.home.curated --width 1024`): 2 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
Catalog snapshot `b3676ad19cdf1f32…` (9 categories, 1816 resources) bound to the design's `AV_*` globals; admin globals bound: AV_RETAINED_OPERATIONS, AV_RETAINED_CATALOG, AV_TOTAL_USERS, AV_USERS, AV_RECENT_ACTIVITY, AV_PENDING_APPROVALS, AV_PENDING_RESOURCES, AV_AUDIT_LOGS, AV_AUDIT_TOTAL, AV_CONTACT_SUBMISSIONS.
