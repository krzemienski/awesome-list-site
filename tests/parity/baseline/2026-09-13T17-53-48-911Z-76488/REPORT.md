...
Run: `2026-09-13T17-53-48-911Z-76488`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2039588655` "Nick" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-13T17:53:00.000Z` on both sides.  
...
Selected run (`--only app.shell.mobile-drawer --width 375`): 1 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
Catalog snapshot `fb62e381c43080d7…` (9 categories, 1817 resources) bound to the design's `AV_*` globals; admin globals bound: AV_TOTAL_USERS, AV_USERS, AV_RECENT_ACTIVITY.
