...
Run: `2026-09-16T03-59-37-695Z-44911`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2070832135` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 3 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T03:59:00.000Z` on both sides.  
...
Selected run (`--only app.admin.users --width 375`): 1 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.users | 375 | pixel | FAIL | 49950 | 9.3539% | 375×1326 vs 375×1424 | [actual](/actual/app.admin.users-375.png) · [expected](/expected/app.admin.users-375.png) · [diff](/diff/app.admin.users-375.png) — 9.354% differing pixels exceeds the 0.5% ceiling |
...
| admin.jsx | `sub="2 admins · 1 contributor"` | `sub="5 admins · 6 contributors"` | /api/admin/users roles (admin session only) |
...
| admin.jsx | `        <Stat label="Tables" value="12" />
        ...
        <Stat label="Rows" value="12" />
        <Stat label="Disk" value="29 MB" />
        <Stat label="Migrations" value="27" sub="0 pending" accent />` | /api/admin/stats database (pg_catalog table count, n_live_tup sum, pg_database_size, drizzle journal; admin session only) |
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","22h ago"],["categories","9","96 KB","28d ago"],["subcategories","92","56 KB","28d ago"],["users","11","80 KB","0s ago"],["resource_audit_log","6,221","2.7 MB","3h ago"],["enrichment_jobs","26","168 KB","22h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
