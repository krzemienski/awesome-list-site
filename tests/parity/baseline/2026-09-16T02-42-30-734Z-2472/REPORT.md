...
Run: `2026-09-16T02-42-30-734Z-2472`  
Claim: **incomplete-evidence**  
Gate: **NOT PASSED** (exit code 2)  
Identity: disposable Clerk admin `2052538878` "Nick" (torn down: local row deleted, Clerk user deleted, 5 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T02:42:00.000Z` on both sides.  
Denominator: 0 pass / 3 fail / **3** pixel rows executed. 1 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.category,app.subcategory --width 375,1440`): 4 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.category | 375 | pixel | FAIL | 447838 | 10.1793% | 375×11732 vs 375×11204 | [actual](/actual/app.category-375.png) · [expected](/expected/app.category-375.png) · [diff](/diff/app.category-375.png) — 10.179% differing pixels exceeds the 0.5% ceiling |
| app.category | 1440 | pixel | FAIL | 435458 | 6.9774% | 1440×4334 vs 1440×4214 | [actual](/actual/app.category-1440.png) · [expected](/expected/app.category-1440.png) · [diff](/diff/app.category-1440.png) — 6.977% differing pixels exceeds the 0.5% ceiling |
| app.subcategory | 375 | pixel | FAIL | 415977 | 9.9468% | 375×11152 vs 375×10676 | [actual](/actual/app.subcategory-375.png) · [expected](/expected/app.subcategory-375.png) · [diff](/diff/app.subcategory-375.png) — 9.947% differing pixels exceeds the 0.5% ceiling |
...
Catalog snapshot `f512129bbeedd235…` (9 categories, 1816 resources) bound to the design's `AV_*` globals; admin globals bound: AV_RETAINED_OPERATIONS, AV_RETAINED_CATALOG, AV_TOTAL_USERS, AV_USERS, AV_ADMIN_RESOURCES, AV_RECENT_ACTIVITY, AV_PENDING_APPROVALS, AV_PENDING_RESOURCES, AV_AUDIT_LOGS, AV_AUDIT_TOTAL, AV_CONTACT_SUBMISSIONS.
...
| admin.jsx | `sub="2 admins · 1 contributor"` | `sub="9 admins · 4 contributors"` | /api/admin/users roles (admin session only) |
