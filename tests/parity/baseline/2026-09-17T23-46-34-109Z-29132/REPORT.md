...
Run: `2026-09-17T23-46-34-109Z-29132`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 0)  
Identity: disposable Clerk admin `2129707650` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-17T23:46:00.000Z` on both sides.  
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 6ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 121d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
