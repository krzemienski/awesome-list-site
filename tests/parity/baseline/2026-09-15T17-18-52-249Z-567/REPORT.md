...
Run: `2026-09-15T17-18-52-249Z-567`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2044760120` "Nick" (torn down: local row deleted, Clerk user deleted, 0 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-15T17:18:00.000Z` on both sides.  
...
Selected run (`--only app.home.index,app.home.curated,app.category,app.about,app.shell.palette --width 375`): 5 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
