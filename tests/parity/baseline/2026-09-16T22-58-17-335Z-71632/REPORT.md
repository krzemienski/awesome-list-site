...
Run: `2026-09-16T22-58-17-335Z-71632`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 0)  
Identity: disposable Clerk admin `2009549053` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 2 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T22:58:00.000Z` on both sides.  
Denominator: 8 pass / 0 fail / **8** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only artifact.docs.buttons,artifact.docs.forms --width 375,768,1024,1440`): 8 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| artifact.docs.buttons | 375 | pixel | PASS | 404 | 0.0294% | 409×3363 vs 409×3363 | [actual](/actual/artifact.docs.buttons-375.png) · [expected](/expected/artifact.docs.buttons-375.png) · [diff](/diff/artifact.docs.buttons-375.png) |
| artifact.docs.buttons | 768 | pixel | PASS | 404 | 0.0191% | 768×2750 vs 768×2750 | [actual](/actual/artifact.docs.buttons-768.png) · [expected](/expected/artifact.docs.buttons-768.png) · [diff](/diff/artifact.docs.buttons-768.png) |
| artifact.docs.buttons | 1024 | pixel | PASS | 248 | 0.0143% | 1024×1696 vs 1024×1696 | [actual](/actual/artifact.docs.buttons-1024.png) · [expected](/expected/artifact.docs.buttons-1024.png) · [diff](/diff/artifact.docs.buttons-1024.png) |
| artifact.docs.buttons | 1440 | pixel | PASS | 248 | 0.0101% | 1440×1709 vs 1440×1709 | [actual](/actual/artifact.docs.buttons-1440.png) · [expected](/expected/artifact.docs.buttons-1440.png) · [diff](/diff/artifact.docs.buttons-1440.png) |
| artifact.docs.forms | 375 | pixel | PASS | 404 | 0.0359% | 406×2771 vs 406×2771 | [actual](/actual/artifact.docs.forms-375.png) · [expected](/expected/artifact.docs.forms-375.png) · [diff](/diff/artifact.docs.forms-375.png) |
| artifact.docs.forms | 768 | pixel | PASS | 404 | 0.0217% | 768×2425 vs 768×2425 | [actual](/actual/artifact.docs.forms-768.png) · [expected](/expected/artifact.docs.forms-768.png) · [diff](/diff/artifact.docs.forms-768.png) |
| artifact.docs.forms | 1024 | pixel | PASS | 248 | 0.0177% | 1024×1370 vs 1024×1370 | [actual](/actual/artifact.docs.forms-1024.png) · [expected](/expected/artifact.docs.forms-1024.png) · [diff](/diff/artifact.docs.forms-1024.png) |
| artifact.docs.forms | 1440 | pixel | PASS | 248 | 0.0124% | 1440×1384 vs 1440×1384 | [actual](/actual/artifact.docs.forms-1440.png) · [expected](/expected/artifact.docs.forms-1440.png) · [diff](/diff/artifact.docs.forms-1440.png) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","41h ago"],["categories","9","96 KB","29d ago"],["subcategories","92","56 KB","29d ago"],["users","10","80 KB","0s ago"],["resource_audit_log","6,223","2.7 MB","6h ago"],["enrichment_jobs","26","168 KB","41h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 3ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 120d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
