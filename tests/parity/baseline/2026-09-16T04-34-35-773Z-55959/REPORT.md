...
Run: `2026-09-16T04-34-35-773Z-55959`  
Claim: **selected-rows-diagnostic**  
Gate: **NOT PASSED** (exit code 1)  
Identity: disposable Clerk admin `2108430937` "Nick Krzemienski" (torn down: local row deleted, Clerk user deleted, 8 __qa_test_parity_ rows remaining)  
Frozen clock: `2026-09-16T04:34:00.000Z` on both sides.  
Denominator: 3 pass / 2 fail / **5** pixel rows executed. 0 row-incomplete, 0 blocked, 0 evidence-only, 0 token-only, 0 aliases, 0 not selected.

Selected run (`--only app.admin.database,app.admin.subcategories,app.admin.resources,app.admin.overview,app.admin.categories --width 1440`): 5 rows executed; rows outside the selection are not listed and the gate is diagnostic only.
...
|---|---:|---|---|---:|---:|---|---|
| app.admin.categories | 1440 | pixel | PASS | 5710 | 0.3930% | 1440×1009 vs 1440×1009 | [actual](/actual/app.admin.categories-1440.png) · [expected](/expected/app.admin.categories-1440.png) · [diff](/diff/app.admin.categories-1440.png) |
| app.admin.database | 1440 | pixel | FAIL | 10690 | 0.5896% | 1440×1259 vs 1440×1259 | [actual](/actual/app.admin.database-1440.png) · [expected](/expected/app.admin.database-1440.png) · [diff](/diff/app.admin.database-1440.png) — 0.590% differing pixels exceeds the 0.5% ceiling |
| app.admin.overview | 1440 | pixel | FAIL | 17610 | 1.3588% | 1440×900 vs 1440×900 | [actual](/actual/app.admin.overview-1440.png) · [expected](/expected/app.admin.overview-1440.png) · [diff](/diff/app.admin.overview-1440.png) — 1.359% differing pixels exceeds the 0.5% ceiling |
| app.admin.resources | 1440 | pixel | PASS | 7152 | 0.2631% | 1440×1888 vs 1440×1888 | [actual](/actual/app.admin.resources-1440.png) · [expected](/expected/app.admin.resources-1440.png) · [diff](/diff/app.admin.resources-1440.png) |
| app.admin.subcategories | 1440 | pixel | PASS | 6044 | 0.2291% | 1440×1832 vs 1440×1832 | [actual](/actual/app.admin.subcategories-1440.png) · [expected](/expected/app.admin.subcategories-1440.png) · [diff](/diff/app.admin.subcategories-1440.png) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[["resources","1,993","14 MB","23h ago"],["categories","9","96 KB","28d ago"],["subcategories","92","56 KB","28d ago"],["users","16","80 KB","0s ago"],["resource_audit_log","6,221","2.7 MB","3h ago"],["enrichment_jobs","26","168 KB","23h ago"]].map((row, i) => (` | /api/admin/stats database.tableStats (count(*), pg_total_relation_size, newest updated_at/created_at vs frozen clock; admin session only) |
...
| admin.jsx | `            {[
            ...
            ].map((row, i) => (` | `            {[{"k":"Database","v":"ready · 2ms","cls":"ok"},{"k":"GitHub sync","v":"failed · 119d ago","cls":"bad"},{"k":"Link checker","v":"no completed runs","cls":"muted"},{"k":"Enrichment queue","v":"5 failed","cls":"bad"},{"k":"Researcher API","v":"healthy","cls":"ok"}].map((row, i) => (` | /api/admin/operations/health, /api/health/ai, /api/github/sync-status|sync-history, /api/admin/link-health/status|history, /api/enrichment/jobs?limit=100 (AdminOverview.renderHealthRows mirror; admin session only) |
