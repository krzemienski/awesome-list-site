...
4. **Everything else works.** Of **64** audited page-level behaviors: **54 PASS, 3 FAIL (FP-01 A-03 + A-04, FP-02 JD-01), 4 BLOCKED (JD-02/03/04 by FP-02 — empty-step seed; C-11 by concrete data-proof — the Category static-resource branch is unreachable because every resource in the current PostgreSQL seed carries a numeric DB id), 2 N/A by FP-03 (H-03, H-04), 1 PARTIAL test-design (S-03), 0 NOTE.** Two Playwright gap-fill passes (`scripts/audit-gap-fill.mjs`, `scripts/audit-gap-fill-2.mjs`) reuse one `browserContext` so the post-login session cookie travels between routes. The first pass converted SR-04/05/06, C-08, S-04, SS-03, R-06b from BLOCKED → PASS; the second pass adds SR-07 (valid submit → success toast), C-09 (Category Suggest-Edit submit), R-04b (ResourceDetail authed Suggest-Edit open + submit), and C-10 (sort + search filter persistence across the full grid → list → compact → grid toggle matrix). See §6.1 for spec details.
...
| C-11 | Static (non-DB) resource external-click branch (`handleResourceClick → window.open(resource.url, '_blank', 'noopener,noreferrer')` at `Category.tsx:387, 397`). Enumerated the entire `/api/awesome-list` payload (9 categories, 703 resources total) and verified `isDbResource(r)` (`Category.tsx:195`) for every resource. Result: `{ totalResources:703, totalDbResources:703, totalNonDbResources:0, categoriesWithNonDb:[] }`. **The static-resource branch is unreachable in the current PostgreSQL-backed dataset** — every resource surfaced through `getAwesomeListFromDatabase()` carries a numeric DB id, so the `window.open` branch is dead code for this seed. Behavior is functionally proven on the equivalent Subcategory path (S-02, where the same `window.open` pattern was driven against the Galène card). | **BLOCKED (concrete data-proof — branch unreachable in current dataset; same `window.open` shape covered by S-02)** | `category/C-11-static-branch-enumeration.json` |
...
├── category/              (14 PNG + 10 *-state.json + C-11-static-branch-enumeration.json — adds C-08, C-09, C-10, C-11 + 4 view-mode JSON pairs C-04a/b/c/d)
