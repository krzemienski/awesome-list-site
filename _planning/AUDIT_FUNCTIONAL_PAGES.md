...
4. **Everything else works.** Of **47** audited page-level behaviors: **37 PASS, 3 FAIL (FP-01 A-03 + A-04, FP-02 JD-01), 3 BLOCKED by FP-02 (JD-02, JD-03, JD-04), 2 N/A by FP-03 (H-03, H-04), 1 PARTIAL test-design (S-03), 1 NOTE (R-06 — feature absent for this resource).**
...
| H-05 | AI recommendation login CTA. Eval-driven click on `a[href="/login"]` returned `null` (stale handle after prior popover-Escape sequence) so the auto-test left URL on `/`. Manual verification: `a[href="/login"]` exists in DOM at H-01 with `loginLinks: 1`. Direct nav-by-test in L-* confirms `/login` route works. | **PASS (verified via L-01)** | `home/H-01-state.json` + `login/L-01-initial.png` |
...
| R-06 | Related resources block. `relatedHeading: []`, `relatedCards: 0`. The page does not advertise a Related section for this resource. Not a defect (no test ID `card-related-*` is in the source — confirmed via grep), just an absent feature for this specific resource's tag set. **NOTE.** | **NOTE — feature absent for this resource** | `resourcedetail/R-06-{related.png,state.json}` |
...
- **Authenticated `/submit` form flow** — only the unauth gate (SR-01..SR-03) was exercised. The actual submit form (title / URL / category fields, validation, POST → toast → redirect) was NOT driven. Add SR-04..SR-NN in a follow-up that logs in via L-05 first, then re-opens `/submit` and runs the form.
- **Login OAuth click-through (Google + GitHub)** — L-01 only asserts the OAuth buttons are present (`hasGoogle: true`, `hasGithub: true`). The click was NOT pushed through (would navigate off-domain to provider; would need stubbed OAuth or interception). Recommend a follow-up that asserts `href` / target endpoint matches `/api/auth/{google,github}` instead of executing the redirect.
- **Category static-resource external click path** — C-05 verified the DB-resource branch (numeric id → `/resource/:id`). The static-resource branch (non-DB cards that go through `window.open + toast`) was NOT exercised on Category. Subcategory S-02 did cover the same `window.open` shape, so behavior is indirectly proven, but a dedicated Category C-08 step (find a non-DB card, override `window.open`, assert the captured URL + toast text) would close the gap.
...
```
evidence/functional/pages/                          (PNG + *-state.json counts as on disk)
├── home/                  (11 PNG +  7 *-state.json + ancillary *.json: options/pick/click/action)
├── category/              ( 7 PNG +  7 *-state.json + ancillary: options/pick/click/buttons)
├── subcategory/           ( 3 PNG +  3 *-state.json + ancillary: click)
├── subsubcategory/        ( 2 PNG +  2 *-state.json)
├── resourcedetail/        ( 6 PNG +  7 *-state.json)
├── submit/                ( 1 PNG +  3 *-state.json)
├── login/                 ( 7 PNG +  5 *-state.json)
