...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/` | Page shell | 9 Layout | `container mx-auto px-4 py-8 max-w-full` (Home.tsx hero block). | DS frame from CC-16. | P1 | Drop `container max-w-full`; rely on `<main>` from CC-16. | `client/src/pages/Home.tsx`. |
...
| `/` | Hero badge | 11 Variants | `<Badge variant="default" class="rounded-full bg-primary">`. | DS `.chip` (DS_SPEC §5.3): square, `--surface-2` bg, 11 px caption. `.chip.live` variant adds `.live-dot`. | P2 | Add `.chip` class to `badge.tsx`. | `client/src/components/ui/badge.tsx` line 7. |
| `/` | Hero heading | 6 Typography (scale) | `text-3xl font-bold` | DS `h1` 40 px (DS_SPEC §8.1) | P2 | Replace with `.t-h1` utility (`text-[40px] leading-tight`). | `client/src/pages/Home.tsx`. |
...
| `/` | Page | 19 States — loading | Phase 2 did not capture; `Home.tsx` shows `<Skeleton>` rows on `isLoading`. | DS `.skeleton` shimmer with `role="status" aria-busy="true"` (DS_SPEC §5.10). | P1 | Add `role="status"`; reuse `client/src/styles/skeleton-animations.css`. | `client/src/pages/Home.tsx`; `client/src/components/ui/skeleton.tsx`. |
| `/` | Page | 20 States — empty | Phase 2 did not capture; `Home.tsx` renders plain `<p>` when no resources. | DS `.empty-state` molecule. | P2 | Add empty-state component. | `client/src/pages/Home.tsx`. |
| `/` | Page | 21 States — error | Phase 2 did not capture; React error boundary uses plain `<p>`. | DS `.alert.error` + `role="alert"`. | P2 | Add `.alert` molecule. | `client/src/pages/AdminDashboard.tsx` lines 48-56 (same pattern). |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/category/:slug` | View-mode toggle | 11 Variants | `<ToggleGroup class="border rounded-md p-0.5">` (`view-mode-toggle.tsx` line 25). | DS `.tabs.compact` (DS_SPEC §5.6) — flat, accent underline. | P1 | Re-skin toggle to `.tabs.compact`. | `client/src/components/ui/view-mode-toggle.tsx` line 25. |
| `/category/:slug` | Resource card (grid/list/compact) | 11 Variants | shadcn `<Card>`; compact = `<div class="flex border-b">`. | DS `.card` + `.card.row` + `.card.compact` (DS_SPEC §5.4). | P1 | Add `.row` / `.compact` modifiers in `design-system.css`. | `client/src/pages/Category.tsx`. |
| `/category/:slug` | "Suggest Edit" inline button | 11 Variants | `<Button variant="ghost" size="icon">` 44×44. | `.btn.icon.ghost` (DS_SPEC §5.1). | P2 | Add `.btn.ghost` modifier. | `client/src/pages/Category.tsx`. |
...
| `/subcategory/:slug` | View-mode toggle | 11 Variants | Same as §4.2. | Same. | P1 | Same PR. | `client/src/pages/Category.tsx` (shared with `/category`). |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/resource/:id` | OG image hero | 2 Tokens (radius) | `aspect-video rounded-lg`. | Square corners (CC-04). | P3 | Drop `rounded-lg`. | `client/src/pages/ResourceDetail.tsx`. |
| `/resource/:id` | Share button | 15 States — focus-visible | shadcn `<Button>` uses `focus-visible:ring-2 focus-visible:ring-ring`. | `2px solid var(--accent) + 2px offset` (DS_SPEC §7). | P1 | Override `focus-visible` in `button.tsx` per CC-02 token swap. | `client/src/components/ui/button.tsx` line 8. |
| `/resource/:id` | Tag chips | 11 Variants | `<Badge variant="secondary">`. | DS `.chip`. | P2 | §4.1 row. | `client/src/pages/ResourceDetail.tsx`. |
| `/resource/:id` | Suggest-Edit modal | 11 Variants + 22 ARIA | `<Dialog>` Radix; `role="dialog" aria-modal="true"` auto. | DS `.modal` (DS_SPEC §5.8) — surface `--surface`, `--line` border, square. | P2 | Re-skin `dialog.tsx` shell. | `client/src/components/ui/dialog.tsx` line 41. |
...
| `/profile` | Avatar | 11 Variants | `<Avatar h-8 w-8>` `rounded-full`. | DS keeps avatars circular by exception (DS_SPEC §5.7). | INFO | Matches DS. | `client/src/components/ui/avatar.tsx`. |
| `/profile` | Fields | 11 Variants | `<Input>` (see §4.10). | `.field`. | P1 | §4.10 row 1. | `client/src/pages/Profile.tsx`. |
...
| `/bookmarks` | Resource cards | 11 Variants | Same `<Card>` as §4.1. | `.card`. | P1 | §4.1 row 2. | `client/src/pages/Bookmarks.tsx`. |
| `/bookmarks` | Empty state | 20 States — empty | Not captured by Phase 2; component renders plain `<p>`. | `.empty-state`. | P2 | §4.1 row 7. | `client/src/pages/Bookmarks.tsx`. |
...
| `/submit` | `<Textarea>` | 11 Variants | `min-h-[80px] rounded-lg` (`textarea.tsx` line 13). | `.field` multi-line variant. | P1 | Same PR. | `client/src/components/ui/textarea.tsx` line 13. |
| `/submit` | Submit button | 11 Variants | `<Button variant="default">` = `bg-primary`. | `.btn.primary` (DS_SPEC §5.1). | P2 | CC-02 token swap. | `client/src/components/ui/button.tsx` line 12. |
| `/submit` | `<FormMessage>` | 21 States — error | Red text default. | DS GAP-7 `.field-error` (caption 11 px `--err`). | P2 | See §7 GAP-7. | `client/src/components/ui/form.tsx`. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/journey/:id` | `<Progress>` | 11 Variants | `h-4 rounded-full bg-secondary` (`progress.tsx` line 15). | DS `.progress` (GAP-2): `--surface-2` track, `--accent` fill, square. | P1 | Drop `rounded-full`; swap colors via tokens. | `client/src/components/ui/progress.tsx` line 15. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/advanced` | Filter chips | 11 Variants | `<AdvancedFilter>` uses `<Badge>`. | `.chip`. | P2 | §4.1 row 3. | `client/src/components/ui/advanced-filter.tsx`. |
| `/advanced` | `<Slider>` thumb | 11 Variants | `h-5 w-5 rounded-full border-2 border-primary` (`slider.tsx` line 21). | Square 16 × 16 thumb, 1-px `--line` border, accent fill on track. | P1 | Override `slider.tsx`. | `client/src/components/ui/slider.tsx` lines 18, 21. |
...
| `/settings/theme` | Accent dot preview | 12 Iconography | `<span class="rounded-full">` (AppHeader.tsx line 113). | 16 × 16 square swatch. | P3 | Drop `rounded-full`; size 16. | `client/src/components/layout/new/AppHeader.tsx` line 113. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/login` | Form | 11 Variants | `<Input>` + `<Button>` same as §4.8. | DS atom swaps. | P1 | §4.8 rows. | `client/src/pages/Login.tsx`. |
| `/login` | OAuth provider button | 11 Variants | `<Button variant="outline">` with provider logo. | `.btn.secondary` + 1.5-stroke lucide icon (GAP-8). | P2 | CC-12 + GAP-8. | `client/src/pages/Login.tsx`. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/about` | Prose | 6 Typography (scale) | Default Tailwind prose sizes. | DS scale `body` 14 / `h2` 28 / `h3` 20. | P2 | Add `.t-body` / `.t-h2` utility classes. | `client/src/pages/About.tsx`. |
...
| `/admin` | Shell `<h1>` | 6 Typography | `text-3xl font-bold text-primary font-mono` (`AdminDashboard.tsx` line 62). | DS `h1` 40 px, `--text` (not `--primary`). | P2 | Replace `text-primary` with `text-[color:var(--text)]`. | `client/src/pages/AdminDashboard.tsx` line 62. |
| `/admin` | `<TabsList>` | 11 Variants | `bg-card border border-primary/20` (`AdminDashboard.tsx` line 73), 14 children. | DS `.tabs` (DS_SPEC §5.6): underline-on-active, no border box. | P1 | Re-skin `tabs.tsx`. | `client/src/pages/AdminDashboard.tsx` lines 73-118. |
| `/admin` | Stats card | 11 Variants | `<Card class="border-primary/20 bg-card">` ×4 (`AdminStats.tsx` lines 25, 39, 53, 67). | DS `.card.stat` + `.eyebrow` label + tabular-nums big number. | P1 | Apply `.card.stat` molecule. | `client/src/components/admin/AdminStats.tsx` lines 25-67. |
| `/admin` | Stat number | 6 Typography | `text-2xl font-bold text-primary font-mono` (`AdminStats.tsx` lines 33, 47, 61, 75). | DS `display`/`display-xl` in `--accent`. | P2 | CC-02 hue; DS scale for size. | `client/src/components/admin/AdminStats.tsx` lines 33-75. |
| `/admin` | Loading state | 19 States — loading | `animate-spin rounded-full border-b-2 border-primary` (`AdminDashboard.tsx` line 41). | `.skeleton` shimmer; spinner OK if `aria-busy="true"`. | P2 | Wrap in `role="status"`. | `client/src/pages/AdminDashboard.tsx` line 41. |
| `/admin` | Error state | 21 States — error | `text-red-500` `<p>` (`AdminDashboard.tsx` line 53). | DS `.alert.error` + `aria-live="assertive"`. | P2 | Use `.alert`. | `client/src/pages/AdminDashboard.tsx` lines 48-56. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| * | `AppSidebar` shell | 11 Variants | shadcn `<Sidebar collapsible="icon" variant="sidebar">` (`AppSidebar.tsx` line 117) — internals = `SidebarMenuButton`, `Collapsible`. | DS `.sidebar` (DS_SPEC §5.11) + nested `.accordion-trigger` / `.accordion-content`. 280 / 64 collapsed. | P1 | Re-skin `client/src/components/ui/sidebar.tsx` to emit `.sidebar` classes; keep shadcn data-attrs. | `client/src/components/layout/new/AppSidebar.tsx` line 117; `client/src/components/ui/sidebar.tsx` lines 38-42, 283. |
| * | Sidebar logo tile | 11 Variants | `bg-primary text-primary-foreground rounded-lg` (`AppSidebar.tsx` line 122). | `--accent` bg, `--accent-ink` fg, square. | P2 | CC-02 + CC-04. | `client/src/components/layout/new/AppSidebar.tsx` line 122. |
| * | Sidebar nav item | 18 States — selected | `data-active=true` → `bg-sidebar-accent`. | DS: 2-px left accent rail + `--accent-mix-15` bg (DS_SPEC §5.11). | P1 | Re-skin via `[data-active=true]` selector in `design-system.css`. | `client/src/components/ui/sidebar.tsx` line 283. |
| * | Sidebar category badge | 11 Variants | `<Badge variant="secondary">` (`AppSidebar.tsx` lines 197, 225). | `.chip` (caption type). | P2 | §4.1 row 3. | `client/src/components/layout/new/AppSidebar.tsx` lines 197, 225. |
| * | Sidebar expand hit-target | 8 Sizing | 44×44 desktop / 36×36 nested sub (`AppSidebar.tsx` lines 231, 287). | Matches DS. | INFO | No work. | `client/src/components/layout/new/AppSidebar.tsx` lines 231, 287. |
| * | Sidebar `/` shortcut | 23 Keyboard | Implemented in `client/src/components/ui/search-dialog.tsx` (keydown listener for `/` and ⌘K/Ctrl+K). | DS_SPEC §7 — `/` focus search, ⌘K palette. | INFO | Matches DS. | `client/src/components/ui/search-dialog.tsx`. |
| * | Mobile drawer | 11 Variants | Sheet portal from shadcn (`<Sidebar>` mobile variant). | DS `.mobile-drawer` (DS_SPEC §5.11). | P2 | Re-skin sheet inner. | `client/src/components/ui/sidebar.tsx`. |
...
| * | Search trigger | 11 Variants | `<button class="rounded-md border bg-transparent">` (`AppHeader.tsx` lines 91-100). | DS `.field.search` + `.kbd`. Square. | P1 | Re-template trigger; reuse `.kbd` on `<kbd>` (line 97). | `client/src/components/layout/new/AppHeader.tsx` lines 91-100. |
| * | Theme button accent dot | 12 Iconography | `rounded-full` 10×10 (`AppHeader.tsx` line 113). | 16 × 16 square swatch. | P3 | Drop `rounded-full`; size 16. | `client/src/components/layout/new/AppHeader.tsx` line 113. |
| * | Breadcrumb | 11 Variants | shadcn `<Breadcrumb>` with slash separator (`breadcrumb.tsx`). | DS `.crumbs` with `›`. | P2 | Re-skin `breadcrumb.tsx`. | `client/src/components/ui/breadcrumb.tsx`. |

### 4.19 Footer (CC component)

| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| * | `<footer>` | 9 Layout | `border-t px-4 sm:px-6 py-3 sm:py-4` (`MainLayout.tsx` line 48). | 1-px `--line` top border, small caption type. | INFO | Matches DS structurally. | `client/src/components/layout/new/MainLayout.tsx` line 48. |
...
| `admin#approvals` | Bulk action buttons | 11 Variants | `<Button variant="default">` row. | `.btn.primary` / `.btn.danger`. | P2 | §5.6 row 1 + CC-02. | `client/src/components/admin/PendingResources.tsx`. |

### 5.2 `admin-edits` (`PendingEdits`)

| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#edits` | Diff view | 11 Variants | Inline diff using `<pre class="bg-muted">`. | `.diff` molecule — DS GAP-3. | P1 | See §7 GAP-3. | `client/src/components/admin/PendingEdits.tsx`. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#enrichment` | `<Progress>` | 11 Variants | `h-4 rounded-full bg-secondary` (`progress.tsx` line 15). | `.progress` square + `--accent` fill (GAP-2). | P1 | §4.10 row 1. | `client/src/components/ui/progress.tsx` line 15. |
...
| `admin#enrichment` | Live pulse | 24 Reduced motion | Pulse ignores `prefers-reduced-motion`. | `.no-anim` honored. | P3 | CC-11. | `client/src/components/admin/BatchEnrichmentPanel.tsx`. |
| `admin#enrichment` | Background job | 19 States — loading | Spinner only; no `aria-busy`. | `role="status" aria-busy="true"` + `.skeleton`. | P2 | Wrap in `<output>`. | `client/src/components/admin/BatchEnrichmentPanel.tsx`. |

### 5.4 `admin-researcher` (`ResearcherTab`)

| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#researcher` | Job table | 11 Variants | shadcn `<Table>` `divide-y border`. | `.table` (DS_SPEC §5.6) — square, hairline rows. | P1 | Re-skin `table.tsx`. | `client/src/components/admin/ResearcherTab.tsx`; `client/src/components/ui/table.tsx`. |
...
| `admin#export` | Export buttons | 16 States — pressed | Default button (no pressed transform). | `transform: translateY(1px)` on `:active`. | P3 | CC-06 + `button.tsx`. | `client/src/components/ui/button.tsx` lines 8-12. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#database` | Danger button | 11 Variants | `<Button variant="destructive">` (`button.tsx` line 13). | `.btn.danger` + confirm modal. | P1 | Token swap + class. | `client/src/components/ui/button.tsx` line 13; `client/src/components/admin/DatabaseTab.tsx`. |
| `admin#database` | Toast on error | 21 States — error | shadcn `<Toast>` (`toast.tsx` line 26 — `shadow-lg`). | `.alert.error` skin. | P2 | Re-skin `toast.tsx`. | `client/src/components/ui/toast.tsx` line 26. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#resources` | Toolbar | 11 Variants | `<Input>` + `<Select>` + `<Button>` mix. | `.toolbar` molecule — DS GAP-5. | P1 | See §7 GAP-5. | `client/src/components/admin/ResourceManager.tsx`. |
| `admin#resources` | Bulk-action dropdown | 11 Variants | shadcn `<DropdownMenu>`. | DS `.menu` — DS GAP-4. | P1 | See §7 GAP-4. | `client/src/components/admin/ResourceManager.tsx`; `client/src/components/ui/dropdown-menu.tsx`. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#categories` | CRUD row | 11 Variants | shadcn `<Table>` + inline edit `<Input>`. | `.table` + `.field` (inline edit). | P1 | §5.4 row 1 + §4.8 row 1. | `client/src/components/admin/GenericCrudManager.tsx`; `client/src/components/admin/CategoryManager.tsx`. |
| `admin#categories` | Confirm modal | 11 Variants | `<AlertDialog>`. | `.modal.confirm` (DS_SPEC §5.8). | P2 | Re-skin `alert-dialog.tsx`. | `client/src/components/ui/alert-dialog.tsx`. |
...
| `admin#subcategories` | Confirm modal | 11 Variants | `<AlertDialog>`. | `.modal.confirm`. | P2 | §5.8 row 2. | `client/src/components/ui/alert-dialog.tsx`. |
...
| `admin#subsubcategories` | Confirm modal | 11 Variants | `<AlertDialog>`. | `.modal.confirm`. | P2 | §5.8 row 2. | `client/src/components/ui/alert-dialog.tsx`. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#github` | Status pill | 11 Variants | Coloured `<Badge>`. | `.chip.ok` / `.chip.warn` / `.chip.err` — DS GAP-1. | P2 | See §7 GAP-1. | `client/src/components/admin/GitHubSyncPanel.tsx`. |
| `admin#github` | Sync log pane | 11 Variants | `<ScrollArea>` over `<pre>` text. | `.log` molecule — DS GAP-6. | P2 | See §7 GAP-6. | `client/src/components/admin/GitHubSyncPanel.tsx`; `_validation/phase-2/admin-github/1280-dark-admin-populated.dom.html`. |
...
| Route | Component | Property | Current value | Target value | Sev | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#linkhealth` | Chart | 11 Variants | `recharts` defaults. | DS palette = `--accent` + tints (DS_SPEC §3.1 mix scale). | P2 | Override `<Cell>` fills to `var(--accent)` + mixes. | `client/src/components/admin/LinkHealthDashboard.tsx`. |
...
| `admin#audit` | Filter row | 11 Variants | `<Select>` + `<Input>`. | `.toolbar` — DS GAP-5. | P2 | See §7 GAP-5. | `client/src/components/admin/AuditTab.tsx`. |
