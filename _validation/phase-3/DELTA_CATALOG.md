...
| CC-08 | * | * | 26 Boot / FOUT | None. `client/index.html` has no inline `data-system` setter; `client/src/main.tsx` runs React after CSS load. | Inline `<script>` in `<head>` per DS_SPEC §6.1 that reads `localStorage.getItem('ds-system' / 'ds-accent')` and sets `document.documentElement.dataset.system/accent` **before** Vite styles load. | Critical (P0) | Edit `client/index.html` (DS_SPEC §10). One inline snippet, ~10 lines. Pair with `applyDesignSystem` import side-effect. | `client/index.html` lines 1-21. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
| CC-09 | * | * | 27 Token API | No `applyDesignSystem` in `client/src/lib/`. | Port from upstream `design-systems.jsx` → `client/src/lib/design-system.ts` (DS_SPEC §10). API: `applyDesignSystem(systemId, accentId)`. Must mutate only DS-owned keys and record `__appliedKeys` on the root (DS_SPEC §6.2). | Critical (P0) | Create the file with types from DS_SPEC §4.1. Side-effect-free import (sets globals lazily). Wire ThemeSettings to it in Phase 4. | `client/src/pages/ThemeSettings.tsx` (existing picker uses a different applier). `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/` | Page shell | 9 Layout | `container mx-auto px-4 py-8 max-w-full` (Home.tsx hero block). | DS frame from CC-16. | High (P1) | Drop `container max-w-full`; rely on `<main>` from CC-16. | `client/src/pages/Home.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
| `/` | `<Card>` (category tile) | 11 Variants | `<Card>` with `rounded-lg shadow-sm bg-card` (`card.tsx` line 12). | DS `.card` (DS_SPEC §5.4): square, 1-px `--line` border, `--surface` bg, padding 16/20. | High (P1) | Add `.card` class to `card.tsx` root. | `client/src/components/ui/card.tsx` line 12; `_validation/phase-2/home/1280-dark-unauth-populated.dom.html`. |
...
| `/` | Hero heading | 6 Typography (scale) | `text-3xl font-bold` | DS `h1` 40 px (DS_SPEC §8.1) | Medium (P2) | Replace with `.t-h1` utility (`text-[40px] leading-tight`). | `client/src/pages/Home.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
...
| `/` | Page | 20 States — empty | Phase 2 did not capture; `Home.tsx` renders plain `<p>` when no resources. | DS `.empty-state` molecule. | Medium (P2) | Add empty-state component. | `client/src/pages/Home.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
| `/` | Page | 21 States — error | Phase 2 did not capture; React error boundary uses plain `<p>`. | DS `.alert.error` + `role="alert"`. | Medium (P2) | Add `.alert` molecule. | `client/src/pages/AdminDashboard.tsx` lines 48-56 (same pattern). `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/category/:slug` | View-mode toggle | 11 Variants | `<ToggleGroup class="border rounded-md p-0.5">` (`view-mode-toggle.tsx` line 25). | DS `.tabs.compact` (DS_SPEC §5.6) — flat, accent underline. | High (P1) | Re-skin toggle to `.tabs.compact`. | `client/src/components/ui/view-mode-toggle.tsx` line 25. `_validation/phase-2/category/1280-dark-unauth-populated.dom.html` |
| `/category/:slug` | Resource card (grid/list/compact) | 11 Variants | shadcn `<Card>`; compact = `<div class="flex border-b">`. | DS `.card` + `.card.row` + `.card.compact` (DS_SPEC §5.4). | High (P1) | Add `.row` / `.compact` modifiers in `design-system.css`. | `client/src/pages/Category.tsx`. `_validation/phase-2/category/1280-dark-unauth-populated.dom.html` |
| `/category/:slug` | "Suggest Edit" inline button | 11 Variants | `<Button variant="ghost" size="icon">` 44×44. | `.btn.icon.ghost` (DS_SPEC §5.1). | Medium (P2) | Add `.btn.ghost` modifier. | `client/src/pages/Category.tsx`. `_validation/phase-2/category/1280-dark-unauth-populated.dom.html` |
...
| `/subcategory/:slug` | View-mode toggle | 11 Variants | Same as §4.2. | Same. | High (P1) | Same PR. | `client/src/pages/Category.tsx` (shared with `/category`). `_validation/phase-2/subcategory/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/resource/:id` | OG image hero | 2 Tokens (radius) | `aspect-video rounded-lg`. | Square corners (CC-04). | Low (P3) | Drop `rounded-lg`. | `client/src/pages/ResourceDetail.tsx`. `_validation/phase-2/resource-detail/1280-dark-unauth-populated.dom.html` |
...
| `/resource/:id` | Tag chips | 11 Variants | `<Badge variant="secondary">`. | DS `.chip`. | Medium (P2) | §4.1 row. | `client/src/pages/ResourceDetail.tsx`. `_validation/phase-2/resource-detail/1280-dark-unauth-populated.dom.html` |
| `/resource/:id` | Suggest-Edit modal | 11 Variants + 22 ARIA | `<Dialog>` Radix; `role="dialog" aria-modal="true"` auto. | DS `.modal` (DS_SPEC §5.8) — surface `--surface`, `--line` border, square. | Medium (P2) | Re-skin `dialog.tsx` shell. | `client/src/components/ui/dialog.tsx` line 41. `_validation/phase-2/resource-detail/1280-dark-unauth-populated.dom.html` |
...
| `/profile` | Fields | 11 Variants | `<Input>` (see §4.10). | `.field`. | High (P1) | §4.10 row 1. | `client/src/pages/Profile.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/submit/1280-dark-unauth-populated.dom.html` |
...
| `/bookmarks` | Resource cards | 11 Variants | Same `<Card>` as §4.1. | `.card`. | High (P1) | §4.1 row 2. | `client/src/pages/Bookmarks.tsx`. `_validation/phase-2/bookmarks/1280-dark-admin-populated.dom.html` |
| `/bookmarks` | Empty state | 20 States — empty | Not captured by Phase 2; component renders plain `<p>`. | `.empty-state`. | Medium (P2) | §4.1 row 7. | `client/src/pages/Bookmarks.tsx`. `_validation/phase-2/bookmarks/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/submit` | `<Input>` | 11 Variants | `h-10 rounded-lg border border-input bg-background` (`input.tsx` line 11). | DS `.field` (DS_SPEC §5.5): square, 1-px `--line` border, 44 px min-height, focus border = `color-mix(in oklab, var(--accent) 60%, var(--line))`. | High (P1) | Override `input.tsx`; add `.field` to `design-system.css`. | `client/src/components/ui/input.tsx` line 11. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/submit/1280-dark-unauth-populated.dom.html` |
| `/submit` | `<Textarea>` | 11 Variants | `min-h-[80px] rounded-lg` (`textarea.tsx` line 13). | `.field` multi-line variant. | High (P1) | Same PR. | `client/src/components/ui/textarea.tsx` line 13. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/submit/1280-dark-unauth-populated.dom.html` |
| `/submit` | Submit button | 11 Variants | `<Button variant="default">` = `bg-primary`. | `.btn.primary` (DS_SPEC §5.1). | Medium (P2) | CC-02 token swap. | `client/src/components/ui/button.tsx` line 12. `_validation/phase-2/submit/1280-dark-unauth-populated.dom.html` |
| `/submit` | `<FormMessage>` | 21 States — error | Red text default. | DS GAP (see §7 GAP-7) `.field-error` (caption 11 px `--err`). | Medium (P2) | See §7 GAP-7. | `client/src/components/ui/form.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/submit/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/advanced` | Filter chips | 11 Variants | `<AdvancedFilter>` uses `<Badge>`. | `.chip`. | Medium (P2) | §4.1 row 3. | `client/src/components/ui/advanced-filter.tsx`. `_validation/phase-2/advanced/1280-dark-unauth-populated.dom.html` |
| `/advanced` | `<Slider>` thumb | 11 Variants | `h-5 w-5 rounded-full border-2 border-primary` (`slider.tsx` line 21). | Square 16 × 16 thumb, 1-px `--line` border, accent fill on track. | High (P1) | Override `slider.tsx`. | `client/src/components/ui/slider.tsx` lines 18, 21. `_validation/phase-2/advanced/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/settings/theme` | System picker | 27 Token API | Uses `useTheme` hook + `client/src/hooks/use-theme.ts`. | DS `applyDesignSystem(systemId, accentId)` from CC-09. Under Option A, render only the Terminal × accent picker (DS_SPEC §10 notes "**Skip** under Option A"). | Critical (P0) | Replace picker action with `applyDesignSystem`; persist `ds-system` + `ds-accent` (CC-08). | `client/src/pages/ThemeSettings.tsx`; `client/src/hooks/use-theme.ts`. `_validation/phase-2/settings-theme/1280-dark-unauth-populated.dom.html` |
| `/settings/theme` | Accent dot preview | 12 Iconography | `<span class="rounded-full">` (AppHeader.tsx line 113). | 16 × 16 square swatch. | Low (P3) | Drop `rounded-full`; size 16. | `client/src/components/layout/new/AppHeader.tsx` line 113. `_validation/phase-2/settings-theme/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/login` | Form | 11 Variants | `<Input>` + `<Button>` same as §4.8. | DS atom swaps. | High (P1) | §4.8 rows. | `client/src/pages/Login.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/submit/1280-dark-unauth-populated.dom.html` |
| `/login` | OAuth provider button | 11 Variants | `<Button variant="outline">` with provider logo. | `.btn.secondary` + 1.5-stroke lucide icon DS GAP (see §7 GAP-8). | Medium (P2) | CC-12 + GAP-8. | `client/src/pages/Login.tsx`. `_validation/phase-2/login/1280-dark-unauth-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `/about` | Prose | 6 Typography (scale) | Default Tailwind prose sizes. | DS scale `body` 14 / `h2` 28 / `h3` 20. | Medium (P2) | Add `.t-body` / `.t-h2` utility classes. | `client/src/pages/About.tsx`. `_validation/phase-2/about/1280-dark-unauth-populated.dom.html` |
...
| `/admin` | Shell `<h1>` | 6 Typography | `text-3xl font-bold text-primary font-mono` (`AdminDashboard.tsx` line 62). | DS `h1` 40 px, `--text` (not `--primary`). | Medium (P2) | Replace `text-primary` with `text-[color:var(--text)]`. | `client/src/pages/AdminDashboard.tsx` line 62. `_validation/phase-2/admin/1280-dark-unauth-gate.dom.html` |
| `/admin` | `<TabsList>` | 11 Variants | `bg-card border border-primary/20` (`AdminDashboard.tsx` line 73), 14 children. | DS `.tabs` (DS_SPEC §5.6): underline-on-active, no border box. | High (P1) | Re-skin `tabs.tsx`. | `client/src/pages/AdminDashboard.tsx` lines 73-118. `_validation/phase-2/admin/1280-dark-unauth-gate.dom.html` |
| `/admin` | Stats card | 11 Variants | `<Card class="border-primary/20 bg-card">` ×4 (`AdminStats.tsx` lines 25, 39, 53, 67). | DS `.card.stat` + `.eyebrow` label + tabular-nums big number. | High (P1) | Apply `.card.stat` molecule. | `client/src/components/admin/AdminStats.tsx` lines 25-67. `_validation/phase-2/admin/1280-dark-unauth-gate.dom.html` |
| `/admin` | Stat number | 6 Typography | `text-2xl font-bold text-primary font-mono` (`AdminStats.tsx` lines 33, 47, 61, 75). | DS `display`/`display-xl` in `--accent`. | Medium (P2) | CC-02 hue; DS scale for size. | `client/src/components/admin/AdminStats.tsx` lines 33-75. `_validation/phase-2/admin/1280-dark-unauth-gate.dom.html` |
| `/admin` | Loading state | 19 States — loading | `animate-spin rounded-full border-b-2 border-primary` (`AdminDashboard.tsx` line 41). | `.skeleton` shimmer; spinner OK if `aria-busy="true"`. | Medium (P2) | Wrap in `role="status"`. | `client/src/pages/AdminDashboard.tsx` line 41. `_validation/phase-2/admin/1280-dark-unauth-gate.dom.html` |
| `/admin` | Error state | 21 States — error | `text-red-500` `<p>` (`AdminDashboard.tsx` line 53). | DS `.alert.error` + `aria-live="assertive"`. | Medium (P2) | Use `.alert`. | `client/src/pages/AdminDashboard.tsx` lines 48-56. `_validation/phase-2/admin/1280-dark-unauth-gate.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `*` | NotFound page | 11 Variants | `<Card>` with copy + back link. | DS `.empty-state` + `.btn.secondary`. | Medium (P2) | §4.1 row 7. | `client/src/pages/not-found.tsx`; `_validation/phase-2/notfound/1280-dark-unauth-notfound.dom.html`. |
...
| * | Mobile drawer | 11 Variants | Sheet portal from shadcn (`<Sidebar>` mobile variant). | DS `.mobile-drawer` (DS_SPEC §5.11). | Medium (P2) | Re-skin sheet inner. | `client/src/components/ui/sidebar.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |

### 4.18 Header / Top-bar (CC component)

| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| * | `AppHeader` shell | 9 Layout | `sticky top-0 z-30 h-14 border-b bg-background/95 backdrop-blur` (`AppHeader.tsx` line 68). | 56 px height OK. Drop backdrop-blur (Terminal forbids blur). Replace `border-b` with `box-shadow: 0 1px 0 var(--line)`. | Medium (P2) | Edit `AppHeader.tsx` line 68 — drop `backdrop-blur supports-[backdrop-filter]:bg-background/60`. | `client/src/components/layout/new/AppHeader.tsx` line 68. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
| * | Search trigger | 11 Variants | `<button class="rounded-md border bg-transparent">` (`AppHeader.tsx` lines 91-100). | DS `.field.search` + `.kbd`. Square. | High (P1) | Re-template trigger; reuse `.kbd` on `<kbd>` (line 97). | `client/src/components/layout/new/AppHeader.tsx` lines 91-100. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
| * | Theme button accent dot | 12 Iconography | `rounded-full` 10×10 (`AppHeader.tsx` line 113). | 16 × 16 square swatch. | Low (P3) | Drop `rounded-full`; size 16. | `client/src/components/layout/new/AppHeader.tsx` line 113. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
| * | Breadcrumb | 11 Variants | shadcn `<Breadcrumb>` with slash separator (`breadcrumb.tsx`). | DS `.crumbs` with `›`. | Medium (P2) | Re-skin `breadcrumb.tsx`. | `client/src/components/ui/breadcrumb.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/home/1280-dark-unauth-populated.dom.html` |
...
| `admin#approvals` | Bulk action buttons | 11 Variants | `<Button variant="default">` row. | `.btn.primary` / `.btn.danger`. | Medium (P2) | §5.6 row 1 + CC-02. | `client/src/components/admin/PendingResources.tsx`. `_validation/phase-2/admin-approvals/1280-dark-admin-populated.dom.html` |

### 5.2 `admin-edits` (`PendingEdits`)

| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#edits` | Diff view | 11 Variants | Inline diff using `<pre class="bg-muted">`. | `.diff` molecule — DS GAP (see §7 GAP-3). | High (P1) | See §7 GAP-3. | `client/src/components/admin/PendingEdits.tsx`. `_validation/phase-2/admin-edits/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#enrichment` | `<Progress>` | 11 Variants | `h-4 rounded-full bg-secondary` (`progress.tsx` line 15). | `.progress` square + `--accent` fill DS GAP (see §7 GAP-2). | High (P1) | §4.10 row 1. | `client/src/components/ui/progress.tsx` line 15. `_validation/phase-2/admin-enrichment/1280-dark-admin-populated.dom.html` |
| `admin#enrichment` | Live status dot | 11 Variants | Static green `<span class="rounded-full">`. | `.live-dot` with pulse (DS_SPEC §5.3). | Medium (P2) | Add `.live-dot` class. | `client/src/components/admin/BatchEnrichmentPanel.tsx`; `_validation/phase-2/admin-enrichment/1280-dark-admin-populated.dom.html`. |
...
| `admin#enrichment` | Live pulse | 24 Reduced motion | Pulse ignores `prefers-reduced-motion`. | `.no-anim` honored. | Low (P3) | CC-11. | `client/src/components/admin/BatchEnrichmentPanel.tsx`. `_validation/phase-2/admin-enrichment/1280-dark-admin-populated.dom.html` |
| `admin#enrichment` | Background job | 19 States — loading | Spinner only; no `aria-busy`. | `role="status" aria-busy="true"` + `.skeleton`. | Medium (P2) | Wrap in `<output>`. | `client/src/components/admin/BatchEnrichmentPanel.tsx`. `_validation/phase-2/admin-enrichment/1280-dark-admin-populated.dom.html` |

### 5.4 `admin-researcher` (`ResearcherTab`)

| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#researcher` | Job table | 11 Variants | shadcn `<Table>` `divide-y border`. | `.table` (DS_SPEC §5.6) — square, hairline rows. | High (P1) | Re-skin `table.tsx`. | `client/src/components/admin/ResearcherTab.tsx`; `client/src/components/ui/table.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/admin-resources/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#export` | Preset cards | 11 Variants | `<Card>` grid. | `.card.stat`. | High (P1) | §4.15 row 4. | `client/src/components/admin/ExportTab.tsx`; `_validation/phase-2/admin-export/1280-dark-admin-populated.dom.html`. |
| `admin#export` | Export buttons | 16 States — pressed | Default button (no pressed transform). | `transform: translateY(1px)` on `:active`. | Low (P3) | CC-06 + `button.tsx`. | `client/src/components/ui/button.tsx` lines 8-12. `_validation/phase-2/admin-export/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#database` | Danger button | 11 Variants | `<Button variant="destructive">` (`button.tsx` line 13). | `.btn.danger` + confirm modal. | High (P1) | Token swap + class. | `client/src/components/ui/button.tsx` line 13; `client/src/components/admin/DatabaseTab.tsx`. `_validation/phase-2/admin-database/1280-dark-admin-populated.dom.html` |
| `admin#database` | Toast on error | 21 States — error | shadcn `<Toast>` (`toast.tsx` line 26 — `shadow-lg`). | `.alert.error` skin. | Medium (P2) | Re-skin `toast.tsx`. | `client/src/components/ui/toast.tsx` line 26. `_validation/phase-2/admin-database/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#resources` | Toolbar | 11 Variants | `<Input>` + `<Select>` + `<Button>` mix. | `.toolbar` molecule — DS GAP (see §7 GAP-5). | High (P1) | See §7 GAP-5. | `client/src/components/admin/ResourceManager.tsx`. `_validation/phase-2/admin-resources/1280-dark-admin-populated.dom.html` |
| `admin#resources` | Bulk-action dropdown | 11 Variants | shadcn `<DropdownMenu>`. | DS `.menu` — DS GAP (see §7 GAP-4). | High (P1) | See §7 GAP-4. | `client/src/components/admin/ResourceManager.tsx`; `client/src/components/ui/dropdown-menu.tsx`. `_validation/phase-2/admin-resources/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#categories` | CRUD row | 11 Variants | shadcn `<Table>` + inline edit `<Input>`. | `.table` + `.field` (inline edit). | High (P1) | §5.4 row 1 + §4.8 row 1. | `client/src/components/admin/GenericCrudManager.tsx`; `client/src/components/admin/CategoryManager.tsx`. `_validation/phase-2/BASELINE_REPORT.md` `_validation/phase-2/admin-resources/1280-dark-admin-populated.dom.html` |
| `admin#categories` | Confirm modal | 11 Variants | `<AlertDialog>`. | `.modal.confirm` (DS_SPEC §5.8). | Medium (P2) | Re-skin `alert-dialog.tsx`. | `client/src/components/ui/alert-dialog.tsx`. `_validation/phase-2/admin-categories/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#subcategories` | CRUD row | 11 Variants | Same as §5.8. | `.table` + `.field`. | High (P1) | Single PR with §5.8. | `client/src/components/admin/SubcategoryManager.tsx`; `_validation/phase-2/admin-subcategories/1280-dark-admin-populated.dom.html`. |
| `admin#subcategories` | Confirm modal | 11 Variants | `<AlertDialog>`. | `.modal.confirm`. | Medium (P2) | §5.8 row 2. | `client/src/components/ui/alert-dialog.tsx`. `_validation/phase-2/admin-subcategories/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#subsubcategories` | CRUD row | 11 Variants | Same as §5.8. | `.table` + `.field`. | High (P1) | Single PR with §5.8. | `client/src/components/admin/SubSubcategoryManager.tsx`; `_validation/phase-2/admin-subsubcategories/1280-dark-admin-populated.dom.html`. |
| `admin#subsubcategories` | Confirm modal | 11 Variants | `<AlertDialog>`. | `.modal.confirm`. | Medium (P2) | §5.8 row 2. | `client/src/components/ui/alert-dialog.tsx`. `_validation/phase-2/admin-subsubcategories/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#users` | Role chip | 11 Variants | `<Badge variant="default">`. | `.chip.admin` / `.chip.user`. | Medium (P2) | §4.1 row 3. | `client/src/components/admin/UsersTab.tsx`; `_validation/phase-2/admin-users/1280-dark-admin-populated.dom.html`. |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#github` | Status pill | 11 Variants | Coloured `<Badge>`. | `.chip.ok` / `.chip.warn` / `.chip.err` — DS GAP (see §7 GAP-1). | Medium (P2) | See §7 GAP-1. | `client/src/components/admin/GitHubSyncPanel.tsx`. `_validation/phase-2/admin-github/1280-dark-admin-populated.dom.html` |
| `admin#github` | Sync log pane | 11 Variants | `<ScrollArea>` over `<pre>` text. | `.log` molecule — DS GAP (see §7 GAP-6). | Medium (P2) | See §7 GAP-6. | `client/src/components/admin/GitHubSyncPanel.tsx`; `_validation/phase-2/admin-github/1280-dark-admin-populated.dom.html`. |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#linkhealth` | Chart | 11 Variants | `recharts` defaults. | DS palette = `--accent` + tints (DS_SPEC §3.1 mix scale). | Medium (P2) | Override `<Cell>` fills to `var(--accent)` + mixes. | `client/src/components/admin/LinkHealthDashboard.tsx`. `_validation/phase-2/admin-linkhealth/1280-dark-admin-populated.dom.html` |
...
| Route | Component | Property | Current value | Target value | Severity | Remediation step | Evidence |
|---|---|---|---|---|---|---|---|
| `admin#audit` | Log table | 11 Variants | `<Table>` with monospace cells. | `.table.mono`. | High (P1) | §5.4 row 1. | `client/src/components/admin/AuditTab.tsx`; `_validation/phase-2/admin-audit/1280-dark-admin-populated.dom.html`. |
| `admin#audit` | Filter row | 11 Variants | `<Select>` + `<Input>`. | `.toolbar` — DS GAP (see §7 GAP-5). | Medium (P2) | See §7 GAP-5. | `client/src/components/admin/AuditTab.tsx`. `_validation/phase-2/admin-audit/1280-dark-admin-populated.dom.html` |
