...
description: |
  Write, review, and improve blog posts for the Sentry engineering blog.
  Use when: Writing blog posts, drafting technical articles, reviewing blog content, improving drafts, product announcements, engineering deep-dives, postmortems for Sentry's developer audience.
  Covers: Sentry voice/tone, banned language list, opening hooks, section structure, title guidelines, technical quality standards, post types (deep dive, launch, postmortem, data, tutorial), editorial review checklists.
  Keywords: blog, sentry, writing, technical article, deep dive, postmortem, announcement, engineering blog, developer writing
---
...
- Non-Sentry blog posts — this skill encodes Sentry-specific voice, standards, and review checklists
- Changelog entries — those belong in the changelog, not blog posts (per Non-Negotiable #8)
- Internal documentation, RFCs, or design docs — use `doc-coauthoring` or `prd` instead
- Social media content (tweets, LinkedIn) — use `copywriting` or `internal-comms`
- Generic technical writing without a Sentry audience — use `writing-skills` instead

## Anti-Patterns

| NEVER | WHY | Fix |
|-------|-----|-----|
| Start with "We're excited to announce" or similar hype | Instant credibility loss; reads like a press release, not engineering blog | State the problem or conclusion in the first sentence |
| Use "Background" / "Architecture" / "Results" as section headings | Conveys zero information; reader skips generic headings | Use headings that make a claim: "Why pre-aggregation destroys debugging context" |
| Make performance claims without numbers | Unsubstantiated claims erode trust; sounds like marketing | Always include the metric: "p99 from 340ms to 45ms — 7.5x improvement" |
| Publish with "The Sentry Team" byline | No accountability; readers want to hear from the person who built it | Always use the real engineer's name who did the work |
| Pad thin content to fill a blog post | Wastes reader time; damages brand reputation for depth | If it's not worth a blog post, put it in the changelog instead |

## Conflicts

- **blog-post-writer** provides general blog writing patterns; this skill enforces Sentry-specific standards and voice
- **copywriting** covers marketing copy and brand voice; this skill is specifically for Sentry engineering blog content
- **humanize-blog-post** post-processes AI-generated content; this skill prevents AI-sounding content from being written in the first place
- **writing-skills** covers general writing craft; this skill adds Sentry-specific quality bars on top

## Related Skills
- `copywriting` — marketing copy and brand voice for non-blog content
- `blog-post-writer` — general blog writing patterns without Sentry-specific standards
- `writing-skills` — general writing craft applicable across all content types
- `internal-comms` — internal status reports and leadership updates (not public blog)
