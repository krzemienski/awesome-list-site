...
- **JSX opening tags are not HTML.** A regex ending at the first `>` truncates on operators such as `>=` inside prop expressions, while requiring `className` immediately after the tag name misses JSX comments. Anchor on a stable attribute, isolate the enclosing tag, and compare individual utility tokens.
