...
- **Clerk server middleware is scoped, not global (Aug 2026):** clerkMiddleware + clerkUserContext run only for `/api/*` and the PROTECTED_PAGE_PATTERNS documents (/admin, /bookmarks, /profile — the server-side page guard needs `req.dbUser` there or signed-in hard loads bounce to /sign-in). Public document navigations skip Clerk server-side — otherwise dev's handshake 307 fired BEFORE index.html (pre-boot scrubber never ran) and headless auditors got stuck on clerk.accounts.dev (Cloudflare 403s the handshake). The client SDK obtains its own dev-browser JWT. If another page route ever reads `req.dbUser`, add it to PROTECTED_PAGE_PATTERNS in server/index.ts or auth silently never resolves.
