...
When a repository query already returns one page slice, the SSR renderer must also receive the authoritative total and must not slice that slice again.
**Why:** treating a 24-row page-2 result as the full collection silently clamps SSR back to page 1, emits “1–24 of 24,” and removes followable pagination even while JSON-LD and the hydrated client correctly describe page 2.
**How to apply:** make slice ownership explicit at the renderer boundary, then validate visible SSR range text and previous/next links—not only canonical and ItemList metadata—on a real page-2 URL.
