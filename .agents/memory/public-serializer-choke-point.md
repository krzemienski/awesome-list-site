...
**Additive fields have the inverse blind spot:** when a NEW public field is attached in the shared serializer, surfaces that never call it get nothing — and those are exactly the repositories that build a hand-picked `select({...})` projection straight from the resources table (published bookmark collections, journey step resources). A walk scoped to "routes that serialize a Resource" misses them. Enumerate by grepping repositories for `.from(resources)` / `innerJoin(resources` with a custom select, attach the fields per row there, and pin each such projection with a strict named response contract so a later edit to the column list fails the contract gate.
