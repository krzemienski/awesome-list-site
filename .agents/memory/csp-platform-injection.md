...
**Headless verification boundary:** once the widget renders, Turnstile's interactive "Verify you are human" checkbox cannot be passed by headless automation (clicks inside the iframe are no-ops) — that final leg needs a real human; probe sign-in health instead via FAPI `POST /api/__clerk/v1/client/sign_ins` (needs_first_factor = healthy).
