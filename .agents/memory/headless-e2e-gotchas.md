...
- Screenshot pipelines can lie: agent-browser captured a "blank" toast that direct Playwright proved renders fine. Before logging a render bug from a screenshot, reproduce with a second capture method.
  - **Why:** Playwright full-page captures can also produce an all-black sticky-header region while the same mounted header is interactive and paints correctly in an element capture. A high crop diff can therefore be a capture failure, not CSS drift.
  - **How to apply:** visually inspect the actual crop before trusting its diff score; require repeat-identical nonblank captures and keep full-canvas acceptance separate from component diagnostics. Successful before/after DOM checks and matching screenshot hashes still do not prove that the PNG contains a populated page. For suspect full-page images, compare a direct CDP full-canvas capture after explicit content readiness; retain the image review as a separate gate.
...
- **Browser “offline” is not proof that a request failed** — an in-flight mutation can still reach the server and return 200. For failure-path tests, abort the exact request with browser routing and confirm both the browser error and absence of a successful server log before asserting the session/data stayed intact.
