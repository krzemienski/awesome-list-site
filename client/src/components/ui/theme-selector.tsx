...
export default function ThemeSelector() {
  ...
  useEffect(() => {
    ...
    if (preferences) {
      ...
      // Apply the loaded theme immediately with the correct variant
      const loadedTheme = shadcnThemes.find((t) => t.value === preferences.theme) || shadcnThemes[6]; // violet is at index 6
      applyTheme(loadedTheme, actualTheme);
      
      // Also sync the mode with ThemeProvider if needed
      if (preferences.mode && preferences.mode !== theme) {
      ...
      const selectedTheme = shadcnThemes.find((t) => t.value === defaultTheme) || shadcnThemes[6];
      applyTheme(selectedTheme, actualTheme);
      
      // Save the default preferences
      saveThemePreferences(defaultTheme, theme);
    }
  }, []); // Remove dependencies to avoid infinite loops

  // Apply theme when variant or actualTheme changes
  useEffect(() => {
    if (themeVariant) {
      const selectedTheme = shadcnThemes.find((t) => t.value === themeVariant) || shadcnThemes[6];
      applyTheme(selectedTheme, actualTheme);
    }
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40">
      ...
      {/* Color Palette Generator Dialog - moved outside Popover to avoid nested modal conflicts */}
