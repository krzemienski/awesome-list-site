    ...
    <div className="taxonomy-results-layout">
      ...
      <div className="min-w-0 flex-1">
        <div ref={resultsRef} tabIndex={-1} className="space-y-4 outline-none" aria-busy={resultsLoading} aria-labelledby="taxonomy-results-heading" data-testid="taxonomy-results-region">
          ...
          {notice && <div role="status" data-testid="notice-page-adjusted" className="rounded border p-3 text-sm">{notice}<button type="button" className="btn-link ml-2 min-h-8" onClick={() => setNotice(null)}>Dismiss</button></div>}
          {(listingData.scope.ignoredSubcategory || listingData.scope.ignoredSubSubcategory) && <div role="status" data-testid="notice-unknown-subcategory" className="rounded border p-3 text-sm">“{selection}” isn't a subcategory of {name}, so that filter was ignored.<button type="button" className="btn-link ml-2 min-h-8" onClick={broadenScope}>Remove it</button></div>}
