...
async function testSubcategoryPagination() {
  ...
  try {
    ...
    const cardCount = await page.$$eval('[class*="ResourceCard"], [class*="resource-card"], .grid > div', 
    ...
    const paginationExists = await page.$('div[class*="pagination"], [role="navigation"]') !== null;
    console.log(`   Pagination visible: ${paginationExists}`);

    if (paginationExists) {
      // Get pagination details
      const paginationText = await page.$eval('div[class*="pagination"] span', el => el.textContent.trim()).catch(() => 'N/A');
      console.log(`   Pagination details: "${paginationText}"`);
    }
    ...
    const cardCount2 = await page.$$eval('[class*="ResourceCard"], [class*="resource-card"], .grid > div', 
      elements => elements.filter(el => {
        const rect = el.getBoundingClientRect();
        return rect.height > 0 && rect.width > 0 && window.getComputedStyle(el).display !== 'none';
      }).length
    ...
    const paginationExists2 = await page.$('div[class*="pagination"], [role="navigation"]') !== null;
    console.log(`   Pagination visible: ${paginationExists2}`);
    ...
    const cardCount3 = await page.$$eval('[class*="ResourceCard"], [class*="resource-card"], .grid > div', 
      elements => elements.filter(el => {
        const rect = el.getBoundingClientRect();
        return rect.height > 0 && rect.width > 0 && window.getComputedStyle(el).display !== 'none';
      }).length
    ...
    const paginationExists3 = await page.$('div[class*="pagination"], [role="navigation"]') !== null;
    console.log(`   Pagination visible: ${paginationExists3}`);

    console.log('\n✅ Test complete! The fix should ensure:');
    console.log('   - Header shows correct paginated range (e.g., "Showing 1-24 of 29 resources")');
    console.log('   - Pagination is always visible for navigation');
    console.log('   - Resource count matches what is displayed');
