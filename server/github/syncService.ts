...
import { AuditRepository, CategoryRepository, GithubSyncRepository, ResourceRepository } from "../repositories";
...
export class GitHubSyncService {
  ...
  async importFromGitHub(
    ...
    try {
      ...
      if (!validationResult.valid) {
        ...
        if (!options.dryRun) {
          await githubSyncRepo.addToGithubSyncQueue({
          ...
          switch (conflict.action) {
            case 'create':
              if (!options.dryRun) {
                ...
                const created = await resourceRepo.createResource({
                  ...conflict.resource,
                  metadata,
                  status: 'approved',
                  githubSynced: true
            ...
            case 'update':
              if (!options.dryRun && conflict.resource.id) {
                ...
                const updated = await resourceRepo.updateResource(
  ...
  async exportToGitHub(
    ...
    try {
      ...
      const currentResources = await resourceRepo.getAllApprovedResources();
      ...
      const lastSync = await githubSyncRepo.getLastSyncHistory(repoUrl, 'export');
      ...
      await Promise.all(
        resourceIds.map(id => 
          resourceRepo.updateResource(id, { 
            githubSynced: true, 
      ...
      await githubSyncRepo.saveSyncHistory({
        repositoryUrl: repoUrl,
        direction: 'export',
  ...
  async processQueue(): Promise<void> {
    ...
    const queueItems = await githubSyncRepo.getGithubSyncQueue('pending');
    ...
    for (const item of queueItems) {
      try {
        ...
        await githubSyncRepo.updateGithubSyncStatus(item.id, 'processing');
        ...
        if (item.action === 'import') {
          ...
          if (result.errors.length === 0) {
            await githubSyncRepo.updateGithubSyncStatus(item.id, 'completed');
          } else {
  ...
  private async checkConflict(resource: Partial<Resource>): Promise<ConflictResolution> {
    ...
    const existingResources = await resourceRepo.listResources({
      limit: 1000,
      status: 'approved'
    });
  ...
  async getSyncHistory(repoUrl: string): Promise<GithubSyncQueue[]> {
    const allItems = await githubSyncRepo.getGithubSyncQueue();
