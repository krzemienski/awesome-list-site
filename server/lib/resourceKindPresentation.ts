...
import { resolveResourceKind } from "@shared/resourceKinds";
import { and, eq, inArray } from "drizzle-orm";
import { db } from "../db";
import { config } from "../config";

type ResourceKindRow = {
  id: number;
  kind?: unknown;
  metadata?: unknown;
};
...
export async function attachResolvedResourceKinds<T extends ResourceKindRow>(
  rows: T[],
): Promise<Array<T & { resolvedKind: ReturnType<typeof resolveResourceKind> }>> {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      resolvedKind: resolveResourceKind(
        typeof row.kind === "string" ? row.kind as any : null,
        [
