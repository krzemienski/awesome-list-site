...
| 9 | `server/config.ts` lacked `resource_kinds` / `contact` types; YAML had no defaults | **Complete**: types + defaults + YAML blocks; `contact.enabled` env-only (`CONTACT_ENABLED=true`). Review round 1 found the env-seeded defaults were then overwritten by the blank YAML block; **fixed** with `resolveContactConfig()` (env > YAML > default per field, blank env = absent) + `tests/unit/contact-config-precedence.test.ts` (10 cases, including re-importing the module against the checked-in YAML so the merge path itself is pinned) | See `assumptions/foundation.md` §2. |
