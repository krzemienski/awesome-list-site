...
### Vendored surface: keep it whole (settled, do not re-litigate)

**Decision.** Generated shadcn files keep their **full upstream export surface**, even
for members this app never renders (`SelectScrollUpButton`, `DropdownMenuRadioGroup`,
`SidebarInput`, `AlertDialogPortal`, …). They are pinned **permanently** in
`scripts/validation/dead-exports-allowlist.json`; a dead-export sweep must leave those
entries alone rather than deleting the symbols.

Why:

- **They are generated, not written.** A shadcn primitive ships as one file exporting
  the whole family. Trimming members makes the next `shadcn add` / upstream refresh of
  that primitive conflict, and whatever was trimmed comes back by hand every time.
- **They cost nothing at runtime.** Rollup tree-shakes unused ESM exports out of the
  production bundle, so an unrendered primitive is not shipped to visitors.
- **They are noise against the signal.** The dead-export gate exists to catch app code
  whose last caller disappeared; ~50 permanent vendored entries would otherwise be
  re-triaged in every sweep to reach the same answer.

**Scope of the exemption** — files generated by the shadcn CLI under
`client/src/components/ui/`, plus `client/src/hooks/use-toast.ts` (also generated).
It does **not** cover the app-specific composites that live in the same folder
(`consent-banner.tsx`, `paginator.tsx`, `skeletons.tsx`, …, see below): those are our
code and follow the normal rule — un-export it or delete it.
