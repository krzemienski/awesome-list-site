    ...
    <div className="brand-kit-page antialiased selection:bg-[var(--accent)] selection:text-[var(--text)] p-12 lg:p-24 pb-32">
      ...
      <div className="max-w-[1000px] mx-auto space-y-32">
        <header className="space-y-4">
          <p className="brand-kit-display italic font-medium uppercase tracking-[3px] brand-kit-accent-tint text-sm">Brand Identity · Color & Typography</p>
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {/* DS-OK: fixed color specimens document the canonical brand palette. */}
            ...
            {/* DS-OK: fixed color specimens document canonical status colors, not reusable UI paint. */}
        ...
        <section className="space-y-12">
          ...
          <div className="flex flex-col md:flex-row h-24 rounded-[var(--radius)] overflow-hidden border brand-kit-hairline">
            {/* DS-OK: the ramp is the fixed brand color reference this page documents. */}
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
            {/* DS-OK: fixed foregrounds are required to document measured brand contrast ratios. */}
          ...
          {/* DS-OK: literal values are explanatory brand guidance, not presentation paint. */}
          <p className="font-mono text-sm brand-kit-muted">Note: Crimson #ff3d52 on black: AA for large text & UI components; use #ffb4be for small crimson text.</p>
        </section>
          ...
          <div className="space-y-24">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="col-span-1">
                <p className="font-mono text-sm brand-kit-muted">Inter 800<br/>Display, -2px tracking</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="col-span-1">
                <p className="font-mono text-sm brand-kit-muted">Inter 400/500<br/>Body paragraph</p>
              </div>
              <div className="col-span-3">
                <p className="font-sans text-lg brand-kit-muted leading-relaxed max-w-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="col-span-1">
                <p className="font-mono text-sm brand-kit-muted">Fraunces italic 500/600<br/>Eyebrow, 3px tracking</p>
              </div>
              <div className="col-span-3">
                <p className="brand-kit-display italic font-medium uppercase tracking-[3px] brand-kit-accent-tint">CURATED · 2,600+ RESOURCES</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="col-span-1">
                <p className="font-mono text-sm brand-kit-muted">JetBrains Mono<br/>Code & Data</p>
              </div>
              <div className="col-span-3 space-y-4 font-mono brand-kit-ink">
...
function ColorSwatch({ name, hex, oklch, color, textColor = "var(--text)", bg, border }: any) {
  return (
    <div className="brand-kit-card overflow-hidden flex flex-col">
      <div
        className="h-32 w-full p-4 flex items-end"
        style={{ backgroundColor: bg || color, borderBottom: border ? 'var(--hairline-w) solid var(--border-strong)' : 'none' }}
      >
        <div className="w-8 h-8 rounded-full shadow-inner" style={{ backgroundColor: color, border: border ? 'var(--hairline-w) solid var(--border-strong)' : 'none' }} />
      </div>
      <div className="p-4 space-y-1 brand-kit-surface">
        <div className="font-sans font-semibold text-sm">{name}</div>
        <div className="font-mono text-xs brand-kit-muted">{hex}</div>
        <div className="font-mono text-[10px] text-[var(--text-3)]">{oklch}</div>
      </div>
    </div>
  );
}
...
function ContrastCard({ fg, ratio, rating }: any) {
  return (
    <div className="brand-kit-card p-5 flex flex-col justify-between h-32">
      <div className="font-sans font-semibold" style={{ color: fg }}>Text Sample</div>
      <div className="flex justify-between items-baseline">
        <div className="font-mono text-sm brand-kit-muted">{ratio}</div>
        <div className="font-mono text-[10px] brand-kit-status-ok px-2 py-0.5 rounded-full">{rating}</div>
      </div>
    </div>
  );
}
