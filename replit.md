...
### Black-Box Audit Remediation — Run21 / Run4 audit (July 19, 2026)
- **81-finding audit triaged live** (R4-001..081, `attached_assets/Pasted--remediation-mission-…1784467678616.txt`): 55 fixed (code), 20 fixed-data (dev applied via admin API, prod journaled), 2 partial/by-design (R4-031 shared-store limiter user-declined — Retry-After + `/api/awesome-list` limiter landed; R4-065 remaining http:// hosts verified https-unsupported), 1 by-design (R4-066 wayback — ALL 14 originals live-probed dead/soft-vanished, 14/14 KEEP, `evidence/run21/wayback-review.md`), 1 invalid (R4-021 IEEE bot-block FP), 2 platform (R4-029 www DNS = registrar action; R4-061 :443 origin redirect = hosting edge). Full table: `evidence/run21/findings-table.md`.
- **Highlights**: R4-019 bcrypt-hash leak closed via field-whitelist `sanitizeUser` on ALL user-returning send sites; R4-014/015/016/047/048/049/076 shared visible-string/URL validator (`shared/validation.ts` + `server/validation/inputs.ts`) across every write path (ZWSP passwords/names, whitespace titles, desc bounds, 2048 URL cap, credential-embedded URLs); R4-030/063 og-image mints nothing — `?path=` resolved from stored data, any title param → byte-identical brand default, `%00` path → 400; R4-033 3.1MB catalog fetched exactly once (all query sites converged on one cache key + early-fetch-aware fetcher); R4-009/028/062 URL canonicalization class (trailing-slash/index.html/case-variant → 301; architect review caught an open-redirect regression — `//evil.com/` minted a protocol-relative Location — fixed with a leading-slash collapse guard, probes in `evidence/run21/open-redirect-guard.txt`); R4-010 hydration head-dedup (1 title + 1 description; note: a 2nd `<title>` string in served HTML is inside a bundled JS lib, not head); R4-013/038/039/070 print stylesheet overhaul; R4-056/057 mutation-error toast class (401 sign-in action / 500 destructive, aria-disabled preserved); R4-081 cross-tab sync (`client/src/lib/crossTabSync.ts`); R4-001/023 Link Health gained domain-takeover + redirect-to-root-at-200 detection.
- **Data corpus** (idempotent, admin-API): 20+ link repoints for takeover/parked/404 domains (afterglow→GitHub, WinFF→GitHub, Kurento→readthedocs, etc.), 65 description backfills, [vc_row] strips, boilerplate rewrites, 6 re-parented resources, 42 whitespace trims, 2 empty taxonomy buckets removed, 7 http→https upgrades (301-verified).
- **Verified** (Iron Rule): tsc clean; migration-drift clean; P0 smoke 17/17 (desktop@1440, mobile@375, auth+journey toggle, admin tabs — `evidence/run21/p0-smoke.md`); server-behavior findings re-curled at final gate; QA teardown net-zero (`__qa_test*` = 0).
- **Prod follow-ups DONE (July 19, 2026, post-republish)**: `scripts/run21-data-fixes-prod.ts` + `scripts/run21-link-fixes-prod.ts` executed against the live admin API — data pass applied (154 fixes confirmed on fresh reads), link pass 42 actions 0 failures; second runs fully no-op; the 2 empty taxonomy buckets are gone from the prod tree; spot-checks (Rocute, UHD Forum→SVTA, IMSC validator, Lightworks, SoX/Audacity https) all confirmed live. Journals: `evidence/run21/data-fixes-prod.json`, `evidence/run21/link-fixes-prod.json`. Remaining user action for R4-029: add `www` DNS record at the registrar (or decline). R4-061 documented as hosting-edge behavior.

### Cold First-Load Perf, Heavy Category Pages — Run21 / BUG-024 carry-over (July 19, 2026)
- **Four client-side changes, no server/schema changes**: (1) `/api/auth/user` early-fetched from inline `client/index.html` script (window.__authUserEarlyFetch) alongside the existing awesome-list early fetch — `useAuth` consumes the promise once via custom queryFn; (2) app-wide auth skeleton gate removed from `App.tsx` — public routes render immediately, AuthGuard/AdminGuard own their loading UI; (3) `processAwesomeListData` memoized (WeakMap) — was re-processing 2,300 resources every render in App + each category page; (4) route-level code splitting — only hot surfaces (Home, 3 category levels, Categories, ResourceDetail, 404) stay in the entry chunk; 17 routes lazy (auth forms, Profile, Bookmarks, Submit+zod, Journeys, Search, Settings, Theme, legal) behind a Suspense skeleton fallback.
- **Main bundle 953KB → 734KB** (gzip 271→220KB; cold-path JS wire 258→211KB). Throttled A/B (100ms RTT, React-cards-visible metric, 3 passes/build): auth request start ~1.2–1.5s → ~0.17–0.37s (parallel with awesome-list); before, card paint was serialized ~340–710ms behind the auth request; after, no auth dependency. Full methodology + tables: `evidence/run21/findings.md`.
- **Measurement gotcha (future audits)**: og-middleware injects full SEO content (h1 + resource links) for ALL user agents — visible ~714ms throttled until React mount wipes it. Perf probes waiting on `a[href^="/resource/"]` can match injected HTML; React paint = wait for `[data-testid^="card-resource"]`.
- **Verified** (Iron Rule): tsc clean; migration-drift clean; 21-route sweep on prod build (all 17 lazy routes render, zero page errors); dev /journeys lazy route renders with full chrome.
- **Needs republish. Post-republish**: run `node evidence/run21/measure-cold.mjs` — expect all three heavy pages (`intro-learning` was 2206ms at baseline, 1572–1755ms re-measured July 19) comfortably under 2s, and `/api/auth/user` starting within ~50ms of `/api/awesome-list`.

### Prod Catch-Up + Run2 Re-Verification — Run20 (July 18, 2026)
- **All journaled prod follow-ups executed** (post-Run19 republish): `run17-data-fixes-prod.ts`, `run18-data-fixes-prod.ts`, `run19-data-fixes-prod.ts`, `run19-retitle-dup-titles.ts`, `run19-dedupe-subsubcats.ts` all run against the live admin API (idempotent; second runs no-op), plus the pending GitHub export re-run — `github_sync_history` now records a completed export. **No prod follow-ups outstanding** except the old failed-export history row (separate task).
- **All 60 NB findings (Run2 audit) re-verified live on prod**: 46 PASS · 9 data-fix confirmed applied · 1 platform (NB-030 edge caching). Full table: `evidence/run20/findings-table.md`; 32 evidence files in `evidence/run20/`.
- **3 regressions found, root-caused, fixed, and re-verified live post-republish**: NB-002 (drawer Escape dropped focus to body — sidebar focus-return re-landed), NB-022 (bookmark/favorite toggles regressed to hard `disabled` during pending, destroying focus — aria-disabled pattern restored), NB-019 (rows-per-page dropdown jumped page scroll 713→0 and rendered off-screen — root cause was `design-system.css` `html, body {height:100%}` capping body height so Radix scroll-lock collapsed the scroll range; fixed to `html{height:100%} body{min-height:100%}` — this class of bug affects ALL Radix popovers/dialogs, keep the rule split).
- **NB-039 completed**: new `DELETE /api/user/submissions/:id` endpoint withdraws the user's own still-pending resource submission (owner-only 403, pending-only 409, audit-before-delete) + withdraw button with confirm dialog in Profile pending-submissions; live on prod.
- **Researcher budget max cap removed** (user request): no upper bound on per-job budget input.
- **NB-020 note for future audits**: Radix tablist shows `tabindex="-1"` on all non-active tabs — that is correct roving-tabindex behavior, not a bug; Tab reaches the tablist and forwards focus to the active tab, arrows move.
- **Verified** (Iron Rule): tsc clean; migration-drift clean; post-republish prod spot-check 4/4 (`r20_prod_postpublish.json`); QA teardown net-zero (`__qa_test*` = 0; external auditor `qa-*` accounts left untouched).
- **New skill**: `.agents/skills/awesome-video-site-audit/` — authoritative audit/fix playbook for this site (surface map, per-domain checklists, gotcha/false-positive database, verification harness) wired to `functional-validation`, `ui-experience-audit`, `visual-inspection`, `playwright-skill`.
