...
- **Dev-only**: prod still carries its own empty taxonomy rows (hidden from users by the `AppSidebar` empty-node filter); a matching pass computed against prod's own tree/IDs should run after the next publish. Dev row IDs do not transfer to prod.
