...
- **Needs republish. Prod follow-ups after republish**: run `scripts/run21-data-fixes-prod.ts` + `scripts/run21-link-fixes-prod.ts` (idempotent, live admin API — prod DB not agent-writable; validated end-to-end against dev via the same code path). User action for R4-029: add `www` DNS record at the registrar (or decline). R4-061 documented as hosting-edge behavior.
