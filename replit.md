...
- **Task #181 client-only sweep** (spec `.local/tasks/task-181.md`): R4-017, R5-017/018/024/025/043, NB-024/059 latest-wins toggles, BUG-006/012/015(verify)/021/024(measure)/038/048. Evidence: `fix-evidence-run24c/verification.md` + real-browser repros at 1440+375.
- **Latest-wins toggle pattern** now on ALL favorite/bookmark/journey-step surfaces: clicks during an in-flight request record the newest desired state in a ref + flip the cache optimistically; `onSettled` fires ≤1 converging follow-up. Critically, `ResourceDetail.tsx` has its own inline favorite/bookmark mutations (NOT the shared FavoriteButton/BookmarkButton) — the audited surface; both converted. Verified: 3 rapid clicks under 1.5s latency → exactly 2 requests (POST+DELETE), UI==server; journey step 3 clicks → 1 PUT.
- **Others verified live**: BUG-021 XSS-param strip boot script (first head script, replaceState); BUG-006 Login replit.com no-cors probe with 4s optimistic timeout, warning only on fetch rejection; R5-024 Categories error card + retry (Sort hidden on error/empty); BUG-024 h1 620ms/list 913ms; BUG-012 dropdown break-all fits at 375 (0px overflow); BUG-038 zero console errors on anon 401 paths; BUG-048 all footer/body links ≥24px tap height; NB-001 chunk guard regression-free (1 auto-reload → retry card).
- **Repro gotcha**: Playwright `.click()` actionability waits serialize "rapid" clicks ~2s apart — dispatch real MouseEvents for rapid-click races. tsc clean; QA teardown net-zero.
- **Review follow-ups**: BUG-006 preflight now fails CLOSED (AbortController 4s; only a successful probe redirects — hanging network → warning, no redirect); R5-018 chunk-reload guard stores `timestamp|href` so only a repeat failure on the SAME URL within 60s falls to the retry card, while a different route gets its own one-shot reload (retry button stamps same format, never clears — NB-001 trap intact). Both re-verified live.
