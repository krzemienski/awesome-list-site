...
- **Needs republish. Prod follow-ups after republish**: rerun `scripts/run15-data-fixes.ts` (BUG-009/011/046); user adds `www` CNAME (BUG-020); dedupe `lower(email)` collisions + add unique functional index on `users(lower(email))`.
