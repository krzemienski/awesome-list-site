...
The Awesome.Video DS contract documented in `docs/` + `HANDOFF.md` is implemented with four intentional, in-repo divergences from the canonical handoff. Every future Stage-5/Stage-6 DS sweep should treat these as architectural decisions, not per-occurrence violations:
