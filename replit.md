...
- **Needs republish. Prod follow-ups after republish**: run `ADMIN_PASSWORD=… npx tsx scripts/run23-data-fixes-prod.ts` (via live admin API — prod DB not agent-writable; validated end-to-end against dev via the same code path).
