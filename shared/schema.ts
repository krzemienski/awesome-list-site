...
/**
 * Legacy HTTP session storage table
 *
 * This stored session data for the pre-Clerk auth stack
 * (Passport.js + connect-pg-simple). Auth sessions now live in Clerk cookies;
 * nothing reads or writes this table anymore.
 *
 * Decision (Task 310, Aug 2026): KEEP the table for historical data and
 * schema stability — dropping it would require a destructive migration for
 * zero operational gain. Revisit only if a future schema cleanup batches
 * several table drops together.
 *
 * The columns are retained for schema and historical-data stability only.
 *
 * Indexes:
...
/**
 * Application users and authorization roles
 *
 * Clerk owns authentication, credentials, and sessions. For migrated users,
 * `id` is the legacy bridge id carried in Clerk's `externalId`/session claims;
 * first-time Clerk users are provisioned here by `server/clerkAuth.ts`.
 *
 * @property {string} id - Clerk bridge id; defaults to a UUID for non-auth seed/data paths
 * @property {string} email - Identity snapshot used for display and collision detection
 * @property {string} password - Legacy nullable column; not used by Clerk authentication
...
export const users = pgTable("users", {
  ...
  password: varchar("password"), // Legacy pre-Clerk hash; not used for sign-in
...
/**
 * Legacy password-reset token table retained for schema stability.
 *
 * Clerk now owns password reset. The application no longer creates or redeems
 * these rows.
 */
