...
// Resource Embeddings - Store vector embeddings for semantic similarity
export const resourceEmbeddings = pgTable(
  ...
  (table) => [
    index("idx_resource_embeddings_resource_id").on(table.resourceId),
    unique("resource_embeddings_resource_id_unique").on(table.resourceId),
  ]
);

export const insertResourceEmbeddingSchema = createInsertSchema(resourceEmbeddings).pick({
  resourceId: true,
  embedding: true,
  model: true,
});

export type InsertResourceEmbedding = z.infer<typeof insertResourceEmbeddingSchema>;
export type ResourceEmbedding = typeof resourceEmbeddings.$inferSelect;

// Related Resources Cache - Store pre-computed related resources for fast loading
export const relatedResourcesCache = pgTable(
  "related_resources_cache",
  {
    id: serial("id").primaryKey(),
    resourceId: integer("resource_id").references(() => resources.id, { onDelete: "cascade" }).notNull(),
    relatedResourceIds: jsonb("related_resource_ids").$type<number[]>().notNull(),
    scores: jsonb("scores").$type<Record<number, number>>().notNull(), // Map of resourceId -> similarity score
  ...
  (table) => [
    index("idx_related_resources_cache_resource_id").on(table.resourceId),
    unique("related_resources_cache_resource_id_unique").on(table.resourceId),
  ]
);

export const insertRelatedResourcesCacheSchema = createInsertSchema(relatedResourcesCache).pick({
  resourceId: true,
  relatedResourceIds: true,
  scores: true,
  metadata: true,
});

export type InsertRelatedResourcesCache = z.infer<typeof insertRelatedResourcesCacheSchema>;
export type RelatedResourcesCache = typeof relatedResourcesCache.$inferSelect;
